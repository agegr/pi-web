...
> Date: 2026-08-12
> Status: spike passed; migration Phases 1-4 are planned but not started
...
Plan diff/protected-file baseline: 58fb9c1486f2f0560ebd9b8ed8d5027cbaa7b0f2
Spike baseline:                  6a76151
```

Execution starts from the documentation commit containing this handoff and the master plan; its exact hash is supplied in the handoff message. `58fb9c1` remains the comparison baseline for protected files and migration commits.
...
- all 40 API paths, methods, status codes, headers, JSON bodies, streams, uploads, and downloads;
...
Import-only adjustments are allowed only if unavoidable. Every phase gate checks these files against `58fb9c1`.
...
- 554 tests passed; lint and `tsc --noEmit` passed.
...
The complete 40-route map, including every method and adapter path, is in the master execution plan. Treat that table as the route inventory source of truth.
...
`src/routes/index.tsx` becomes the real AppShell route. Use TanStack Router search validation for optional `session` and `cwd` strings, and replace the seven Next router replacements in `components/AppShell.tsx` with TanStack `useNavigate` calls.
...
Convert all internal handlers to Web APIs, add all 40 TanStack adapters, install global security middleware, preserve startup ordering, expand runtime smoke coverage, and repeat the SSE gate.

Hard gate: route inventory is exactly 40, no Next API remains in `app/api`, security matrix passes, 310-second SSE passes with middleware, full tests/lint/typecheck/build/verifier/smoke pass, and protected files are unchanged. Stop on failure.
...
Hard gate: deep links and AppShell work, metadata/PWA checks pass, no source imports from `next/*` remain, full tests/lint/typecheck/build/smoke pass, and protected files are unchanged. Stop on failure.
...
Hard gate: `npm pack` of the external staging directory, fresh install of that exact tarball, `bin/pi-web` runtime smoke, runtime dependency resolution, package file audit, Windows CI, full local gates, and protected files unchanged. Stop on failure. Do not publish.

### Phase 4: Functional and cross-platform regression

Run automated and manual workflows for sessions, agent commands/SSE, files, git, auth, models, skills, plugins, worktrees, update checks, security, PWA, offline behavior, and deep links. Update contributor/release documentation to the verified TanStack commands.
...
- exactly 40 API routes are reachable through TanStack Start with unchanged contracts;
...
- tests, lint, typecheck, build verification, smoke tests, and protected-file checks all pass;
