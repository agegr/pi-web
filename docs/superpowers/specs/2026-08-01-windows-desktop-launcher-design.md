...
- Use a built-in Windows icon for the shortcut; do not add or download an icon asset.
...
The installer is idempotent: rerunning it updates the same Desktop shortcut. It uses `-ExecutionPolicy Bypass` only for the shortcut’s one PowerShell process and does not change the computer-wide policy.
