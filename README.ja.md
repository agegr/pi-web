...
ループバック以外のアドレスにバインドすると、高い権限の操作を実行できるエージェントがネットワークに公開されます。信頼できる LAN で使用する場合は、何らかのパスワード（短い PIN でも構いません）を設定してください：
...
**同じ Wi-Fi の場合。** LAN にバインドし、スマートフォンで `http://<コンピューターのLAN IP>:30141` を開きます：
...
```bash
PI_WEB_PASSWORD='任意のパスワード' pi-web --hostname 100.x.x.x
```

スマートフォンで `http://<tailscale-ip>:30141` を開きます。Tailscale クライアント：[iOS](https://apps.apple.com/us/app/tailscale/id1470499037) · [Android](https://play.google.com/store/apps/details?id=com.tailscale.ipn)。他の人と共有するには、同じ tailnet に招待してアドレスを伝えてください。

ループバック以外のアドレスにバインドする場合は、必ず `PI_WEB_PASSWORD` を設定してください。初回ログイン成功后、ブラウザは 30 日有効なセッション cookie を保持するため、cookie の期限切れ、ブラウザデータの消去、デバイスの切り替え時のみパスワードの入力が求められます。
