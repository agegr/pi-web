...
export function useAgentSession(opts: UseAgentSessionOptions) {
    ...
    const checkServerIdle = async () => {
      ...
      try {
        ...
        if (data.running && state?.isCompacting) {
          ...
          setAgentPhase({ kind: "auto_compacting" });
  ...
  const reconcileAgentState = useCallback(async (sid: string) => {
    ...
    try {
      ...
      if (state?.isCompacting) setAgentPhase({ kind: "compacting" });
  ...
  const handleAgentEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      ...
      case "compaction_end":
        ...
        setIsCompacting(false);
        setAgentPhase(event.aborted ? null : { kind: "waiting_model" });
  ...
  const handleAbortCompaction = useCallback(async () => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    compactionAbortRequestedRef.current = true;
    try {
