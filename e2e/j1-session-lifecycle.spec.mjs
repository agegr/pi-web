...
import { BASE, e2eDir, writeLinearSession, writeBranchedSession, cleanupDir, launchPage, waitForText, assertNoBrowserErrors, resultLog } from "./helpers.mjs";
...
try {
  ...
  const hasSentinel = await page.evaluate(() => /load earlier/i.test(document.body.innerText || ""));
