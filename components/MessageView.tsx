...
function ToolCallBlock({ block, result, duration, onOpenSession }: { block: ToolCallContent; result?: ToolResultMessage; duration?: number; onOpenSession?: (sessionId: string) => void }) {
  ...
  const copyTarget = getToolCommandText(block);
      ...
      <div style={{ display: "flex", alignItems: "stretch", minWidth: 0 }}>
        ...
        >
          ...
          <span
            role="button"
            tabIndex={0}
            title={copied ? t("chat.copiedCommand") : t("chat.copyCommand")}
            aria-label={copied ? t("chat.copiedCommand") : t("chat.copyCommand")}
            onClick={(event) => {
            ...
            style={{ display: "grid", placeItems: "center", width: 18, height: 18, flexShrink: 0, color: copied ? "var(--accent)" : "var(--text-dim)", cursor: "pointer" }}
          >
...
function getToolCommandText(block: ToolCallContent): string {
