          ...
          <div style={{ flex: isMobile ? "1 1 auto" : "0 0 auto", minWidth: 0, display: "flex", alignItems: "center", gap: 2 }}>
            ...
            {onWorkspaceHistoryCommand && (
              <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                {(["undo", "redo", "checkpoint"] as const).map((command) => {
                  const labels = { undo: "撤销", redo: "重做", checkpoint: "检查点" };
                  const titles = { undo: "撤销上一轮 Agent 修改", redo: "重做已撤销的修改", checkpoint: "保存当前工作区检查点" };
                  return (
                    <button
                      key={command}
                      type="button"
                      onClick={() => onWorkspaceHistoryCommand(command)}
                      disabled={isStreaming}
                      title={titles[command]}
                      aria-label={titles[command]}
                      style={{
                        display: "flex", alignItems: "center", justifyContent: "center",
                        padding: "0 7px", height: 32, background: "none", border: "none",
                        borderRadius: 9, color: isStreaming ? "var(--text-dim)" : "var(--text-muted)",
                      ...
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = "none";
                        e.currentTarget.style.color = isStreaming ? "var(--text-dim)" : "var(--text-muted)";
                      }}
                    >
                      {labels[command]}
                    </button>
