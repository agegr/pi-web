...
// pi#17 behavior-equivalent guards carried over from the fork's
// jump-to-bottom implementation (superseded by upstream #845, pi#24):
// the button must be invisible AND inert at the tail (no pointer-events
// leak), and must never appear during the hidden scroll-restore window.

test("hidden button is inert, not merely transparent (no pointer-events leak)", () => {
  const hidden = cssSource.slice(
    cssSource.indexOf(".chat-scroll-to-bottom {"),
    cssSource.indexOf(".chat-scroll-to-bottom.is-visible {"),
  );
  assert.match(hidden, /pointer-events: none;/, "hidden state must not intercept pointer events");
  assert.match(hidden, /visibility: hidden;/, "hidden state must be outside the tab order and invisible");
});

test("button stays hidden throughout the scroll-restore window", () => {
  assert.match(
    source,
    /className=\{`chat-scroll-to-bottom\$\{showScrollToBottom && !pendingScrollRestore \? " is-visible" : ""\}`\}/,
    "the pendingScrollRestore window suppresses visibility even when the viewport is detached",
  );
});
...
test("localized label key exists in all three locales", async () => {
  for (const file of ["../lib/i18n/messages/en.ts", "../lib/i18n/messages/zh-CN.ts", "../lib/i18n/messages/zh-TW.ts"]) {
    const locale = await readFile(new URL(file, import.meta.url), "utf8");
    assert.match(locale, /"chat\.scrollToLatest": "(.+)",/, `${file} must define chat.scrollToLatest`);
  }
});
