...
const chatWidthHook = await readFile(new URL("../hooks/useChatContentWidth.ts", import.meta.url), "utf8");
const jiti = createJiti(import.meta.url);
const { clampChatContentWidth } = await jiti.import("../hooks/useChatContentWidth.ts");
...
test("General appearance settings own the chat width preference", () => {
  assert.match(chatWindow, /useChatContentWidth\(\)/);
  assert.match(settingsPanel, /useChatContentWidth\(\)/);
  ...
  assert.match(chatWidthHook, /pi-chat-content-width/);
  assert.match(chatWidthHook, /localStorage\.setItem/);
});
