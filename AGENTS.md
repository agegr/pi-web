...
`GET /api/models` returns `defaultModel` read from `~/.pi/agent/settings.json`. `ChatWindow` pre-selects this on mount for new sessions. Explicit browser model/thinking selections are applied atomically during AgentSession construction (session-scoped only). They must **not** rewrite `~/.pi/agent/settings.json` — that used to make one-off new-session picks silently become the global default (unlike the TUI, where `setModel` only persists when `options.persist` is true). `lib/startup-preferences.ts` remains the hook for a future explicit "Save as default" action; mid-session `set_model` likewise stays non-persisting.
