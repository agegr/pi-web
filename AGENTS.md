...
- auth.json holds **one** credential per provider and `ModelRuntime.logout()` deletes whichever it is. So the delete routes gate on `canRemoveCredential()`, and `ModelsConfig` refreshes *both* provider lists after any auth change — refreshing one leaves a dual-auth provider rendered twice, and the stale row's Disconnect would delete the credential just created.
