...
lib/
  ...
  subagent-dispatch.ts  programmatic subagent dispatch API (globalThis.__piSubagentDispatch)
  subagent-state.ts   globalThis-backed registry of in-flight subagent dispatches
...
`globalThis.__piSubagentDispatch.startSubagentDispatch(parentSessionId, params)` lets extensions dispatch subagents without a tool call; the registry survives hot reload. Per-dispatch params override the profile for tools, disallowed tools, extension allow/deny lists, and `persistSession` — with `persistSession: false` the child runs fully in memory and its audit trail is routed to the parent session. All resource resolution is centralized in `resolveSubagentResources()` (`lib/subagent-dispatch.ts`), shared by the dispatch layer and the runtime; concurrency stays with the upstream SubagentQueue.
