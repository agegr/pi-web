...
- Discovery resolves its endpoint from `models.json` first and falls back to pi's provider catalog, so a models-only entry (no `baseUrl`/`api`) and a built-in provider both work; `resolveModelDiscoveryAuth()` returns that effective `baseUrl`/`api` alongside the credentials, and the discover route and the modal no longer require a configured base URL.
