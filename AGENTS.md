...
- `/api/files` is intentionally not a general filesystem browser. Allowed roots come from session cwds, their resolved project roots, `~/pi-cwd-*`, the configured default directory in `~/.pi/agent/pi-web.json`, and roots explicitly added with `allowFileRoot()`.
