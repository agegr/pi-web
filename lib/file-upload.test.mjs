...
test("validates mkdir folder names as a single bounded path segment", async () => {
  const { validateMkdirFolderName } = await loadSubject();

  assert.equal(validateMkdirFolderName("notes"), null);
  assert.equal(validateMkdirFolderName("my folder 2"), null);
  assert.equal(validateMkdirFolderName(null), "Folder name is required");
  assert.match(validateMkdirFolderName(""), /required/);
  assert.match(validateMkdirFolderName(".."), /Invalid folder name/);
  assert.match(validateMkdirFolderName("."), /Invalid folder name/);
  assert.match(validateMkdirFolderName("a/b"), /must not contain a path/);
  assert.match(validateMkdirFolderName("a\\b"), /must not contain a path/);
  assert.match(validateMkdirFolderName("../escape"), /must not contain a path/);
  assert.match(validateMkdirFolderName("a\0b"), /Invalid folder name/);
  assert.match(validateMkdirFolderName("x".repeat(256)), /too long/);
  // 255 single-byte characters stay within the budget.
  assert.equal(validateMkdirFolderName("x".repeat(255)), null);
});
