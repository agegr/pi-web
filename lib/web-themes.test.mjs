...
describe("PALETTES", () => {
  ...
  it("has all required palettes", () => {
    const required = ["pi", "midnight", "tokyo-night", "catppuccin", "dracula", "nord", "gruvbox", "rose-pine", "solarized", "cyberpunk", "obsidian", "arctic-light", "paper-ink", "amber-terminal"];
  ...
  it("each palette has a group and descriptor", () => {
    for (const palette of PALETTES) {
      assert.ok(["signature", "dark", "light", "special"].includes(palette.group), `${palette.id} group invalid`);
