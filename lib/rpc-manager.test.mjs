...
test("RPC session startup still calls the startup-preferences helper without replaying setters", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // Helper is currently a no-op for global defaults (session-scoped only), but
  // the call site stays as the extension point for an explicit "Save as default".
