...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(startupSource, /getAppModelExtensions\(/);
  assert.doesNotMatch(startupSource, /installCursorModelRuntime\(/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(
    startupSource,
    /chatOnly\s*\?\s*\{[\s\S]*?\.\.\.CHAT_ONLY_RESOURCE_LOADER_OPTIONS[\s\S]*?extensionFactories: getAppModelExtensions\(\)/,
  );
