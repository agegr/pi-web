...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(startupSource, /createScopedAgentSessionServices\(/);
  assert.ok(startupSource.indexOf("createScopedAgentSessionServices(") < startupSource.indexOf("createAgentSessionFromServices("));
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  // Both paths must carry the policy to the pre-import helper, not a post-load filter.
  assert.match(subagentSource, /createScopedAgentSessionServices\(/);
  assert.match(subagentSource, /\}, profile\.extensionScope\)/);
  assert.match(subagentSource, /extensionScope: \[\.\.\.profile\.extensionScope\]/);
  assert.match(startupSource, /\}, subagentResources\?\.extensionScope\)/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.ok(startupSource.indexOf("const chatOnly") < startupSource.indexOf("createScopedAgentSessionServices("));
