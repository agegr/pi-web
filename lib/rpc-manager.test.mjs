...
test("queue mutations rebuild the queue through clearQueue + per-kind requeue", async () => {
  ...
  assert.match(helperSource, /await this\.inner\.prompt\(target, \{ streamingBehavior: "steer", source: "rpc" \}\)/);
...
test("queue-wide interject steers every pending item FIFO per kind", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const steerAllSource = source.slice(source.indexOf("  private async steerAllQueued("), source.indexOf("  private async snapshotQueue("));
