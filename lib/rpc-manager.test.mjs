...
test("TTFT stamps only the turn's first assistant message", async () => {
	const source = await readFile(
		new URL("./rpc-manager.ts", import.meta.url),
		"utf8",
	);
	const ttftSource = source.slice(
		source.indexOf("private handleTtftEvent"),
		source.indexOf("setForceEmptySystemPrompt"),
	);

	assert.match(ttftSource, /ttftStamped/);
	assert.match(ttftSource, /if \(this\.ttftMs > 0 && !this\.ttftStamped\)/);
	assert.match(ttftSource, /this\.ttftStamped = true/);
	// the latch resets alongside ttftMs when a new turn starts
	assert.match(source, /this\.ttftMs = 0;\s*this\.ttftStamped = false;/);
});
