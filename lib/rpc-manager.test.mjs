...
test("Robin sessions keep their declared tool allow-lists exact", async () => {
  ...
  const routeSource = await readFile(new URL("../app/api/robin/assistant/route.ts", import.meta.url), "utf8");
  ...
  assert.match(routeSource, /exactTools: true/);
  assert.equal((routeSource.match(/exact: true/g) ?? []).length, 2);
});
