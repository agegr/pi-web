...
test("discussion threads branch from an assistant and persist non-context metadata", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const threadSource = source.slice(
    source.indexOf('case "start_thread"'),
    source.indexOf('case "set_thinking_level"'),
  );

  assert.match(threadSource, /sourceEntry\.message\.role !== "assistant"/);
  assert.match(threadSource, /getBranch\(hostLeafId\).*sourceEntryId/s);
  assert.match(threadSource, /await this\.inner\.navigateTree\(sourceEntryId, \{\}\)/);
  assert.match(threadSource, /const anchorKey = typeof command\.anchorKey === "string"/);
  assert.match(threadSource, /appendCustomEntry\(DISCUSSION_THREAD_CUSTOM_TYPE/);
  assert.match(threadSource, /\.\.\.\(anchorKey \? \{ anchorKey \} : \{\}\)/);
  assert.match(threadSource, /sessionManager\.buildSessionContext\(\)\.messages/);
});
