...
test("RPC session startup injects the hidden subagent rpc extension and passes its capture to the wrapper", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /createSubagentRpcCapture\(\)/);
  assert.match(startupSource, /extensionFactories: \[subagentRpc\.extension\]/);
