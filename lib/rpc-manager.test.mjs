...
test("clone copies the current active leaf into a child session", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const cloneSource = source.slice(
    source.indexOf('case "clone"'),
    source.indexOf('case "navigate_tree"'),
  ...
  assert.match(cloneSource, /const leafId = sessionManager\.getLeafId\(\)/);
...
test("session replacement rejects active work and clone writes one reopenable child", async () => {
  ...
  try {
    ...
    const firstClone = wrapper.send({ type: "clone" });
    ...
    assert.equal(cloned.getLeafId(), manager.getLeafId());
