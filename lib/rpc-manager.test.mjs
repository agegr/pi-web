...
test("saving the model config reloads every live session's ModelRuntime", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const routeSource = await readFile(new URL("../app/api/models-config/route.ts", import.meta.url), "utf8");

  // Registry-wide reload helper: live sessions only, serial refresh with
  // per-session error tolerance, and a count of successfully refreshed ones.
  assert.match(source, /export async function refreshRpcSessionModelConfigs\(\): Promise<number>/);
  assert.match(source, /filter\(\(wrapper\) => wrapper\.isAlive\(\)\)/);
  assert.match(source, /await wrapper\.inner\.modelRuntime\.refresh\(\{ allowNetwork: false \}\)/);
  assert.match(source, /catch \(error\)/);

  // PUT /api/models-config saves first, then reloads live sessions.
  assert.ok(routeSource.includes("refreshRpcSessionModelConfigs"));
  assert.ok(
    routeSource.indexOf("await refreshRpcSessionModelConfigs()") > routeSource.indexOf("writeModelsConfig(body)"),
    "refresh must run after the config file is written",
  );
});
