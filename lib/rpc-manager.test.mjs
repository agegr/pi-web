...
test("RPC session startup injects the hidden inline extensions and passes the subagent capture to the wrapper", async () => {
  ...
  assert.match(startupSource, /extensionFactories: \[subagentRpc\.extension, reasoningRouter\]/);
...
test("queue mutations rebuild the queue through clearQueue + per-kind requeue", async () => {
  ...
  // Steer must go through send(prompt) so an in-flight model wait is aborted
  // and the item starts a new turn. inner.prompt(steer) only queues and the
  // loop will not drain it until streamAssistantResponse returns.
  assert.match(helperSource, /type: "prompt"[\s\S]*streamingBehavior: "steer"/);
  assert.doesNotMatch(helperSource, /this\.inner\.prompt\(target/);
...
test("steer while streaming aborts the in-flight model wait then starts a new turn", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const sendSource = source.slice(source.indexOf("  async send(command: Record<string, unknown>): Promise<unknown> {"), source.indexOf("  private resolveExtensionUiResponse"));
  const promptCase = sendSource.slice(sendSource.indexOf('case "prompt":'), sendSource.indexOf('case "abort":'));
  assert.match(promptCase, /streamingBehavior === "steer" && this\.inner\.isStreaming/);
  assert.match(promptCase, /this\.inner\.abort\(\)/);
  assert.match(promptCase, /streamingBehavior = undefined/);
});
...
test("queue-wide interject steers every pending item FIFO per kind", async () => {
  ...
  assert.match(steerAllSource, /const all = \[\.\.\.steering, \.\.\.followUp\]/);
  assert.match(steerAllSource, /streamingBehavior: "steer"/);
  assert.match(steerAllSource, /all\.slice\(1\)/);
});
