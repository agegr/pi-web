...
const EXPECTED_ROUTES = {
  ...
  "app/api/sessions/[id]/trajectory/route.ts": ["GET"],
...
const EXPECTED_ADAPTERS = {
  ...
  "src/routes/api/sessions/$id/trajectory.ts": { route: "/api/sessions/$id/trajectory", methods: ["GET"] },
...
test("every API handler has a thin TanStack adapter with the expected route and methods", async () => {
  assert.equal(Object.keys(EXPECTED_ADAPTERS).length, 44);
...
test("the internal API inventory contains exactly the expected 44 routes", async () => {
