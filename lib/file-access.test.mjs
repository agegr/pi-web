...
test("does not scan the user home directory while calculating allowed roots", () => {
  const source = fs.readFileSync(new URL("./file-access.ts", import.meta.url), "utf8");

  assert.doesNotMatch(source, /\bhomedir\b/);
  assert.doesNotMatch(source, /\b(?:readdir|opendir)(?:Sync)?\b/);
});
