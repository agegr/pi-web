...
function makeEnvironment({
  ...
  const env = {
    send: async (subscription, payload, vapidKeys) => {
      const result = { statusCode: statusCodeFor(subscription), subscription, payload, vapidKeys };
      sent.push(result);
      return result;
    },
    loadState: () => state,
...
test("notifySessionComplete dedupes duplicate agent_end events within 5s", async () => {
  const { notifier, sent, setTime } = makeEnvironment();
  notifier.addSubscription(SUB_EN);
  ...
  setTime(4000);
  await notifier.notifySessionComplete("session-1");
  assert.equal(sent.length, 1, "duplicate within the window must be swallowed");

  setTime(7000);
  await notifier.notifySessionComplete("session-1");
  assert.equal(sent.length, 2, "a later run must notify again");
});
...
test("notifySessionComplete prunes subscriptions dropped by the push service", async () => {
  const { notifier, sent, getSaved } = makeEnvironment({
    statusCodeFor: (subscription) => subscription.endpoint === SUB_ZH.endpoint ? 410 : 201,
  });
  notifier.addSubscription(SUB_EN);
  notifier.addSubscription(SUB_ZH);
