...
test("a stable project key migrates a remembered raw Windows key", () => {
  const storage = createStorage({
    "pi-web:last-open-by-workspace": JSON.stringify({
      "C:\\Users\\Alex\\Project": "legacy-session",
      "D:\\Other": "other-session",
    }),
  });

  assert.equal(getLastOpenSession({
    key: "c:\\users\\alex\\project",
    legacyKeys: ["C:\\Users\\Alex\\Project"],
  }, storage), "legacy-session");
  assert.deepEqual(
    JSON.parse(storage.values.get("pi-web:last-open-by-workspace")),
    {
      "c:\\users\\alex\\project": "legacy-session",
      "D:\\Other": "other-session",
    },
...
test("migration accepts a raw cwd spelling distinct from the resolved project root", () => {
  const storage = createStorage({
    "pi-web:last-open-by-workspace": JSON.stringify({
      "c:/users/alex/project/": "raw-cwd-session",
    }),
  });

  assert.equal(getLastOpenSession({
    key: "c:\\users\\alex\\project",
    legacyKeys: ["C:\\Users\\Alex\\Project", "c:/users/alex/project/"],
  }, storage), "raw-cwd-session");
  assert.deepEqual(
    JSON.parse(storage.values.get("pi-web:last-open-by-workspace")),
    { "c:\\users\\alex\\project": "raw-cwd-session" },
  );
...
test("stable-key lookup does not infer POSIX case aliases", () => {
  const storage = createStorage({
    "pi-web:last-open-by-workspace": JSON.stringify({
      "/users/alex/project": "different-directory",
    }),
  });

  assert.equal(getLastOpenSession({
    key: "/Users/Alex/Project",
    legacyKeys: [],
  }, storage), null);
  assert.deepEqual(
    JSON.parse(storage.values.get("pi-web:last-open-by-workspace")),
...
test("writes and clears remove explicitly supplied legacy keys", () => {
  const storage = createStorage({
    "pi-web:last-open-by-workspace": JSON.stringify({ "C:\\Repo": "old" }),
  });
  const workspace = { key: "c:\\repo", legacyKeys: ["C:\\Repo"] };

  setLastOpenSession(workspace, "new", storage);
  assert.deepEqual(
    JSON.parse(storage.values.get("pi-web:last-open-by-workspace")),
    { "c:\\repo": "new" },
  );

  clearLastOpen(workspace, storage);
