...
test("invokes onAgentEnd exactly for agent_end events", async () => {
  const startup = deferred();
  const abortController = new AbortController();
  const agentEndSessionIds = [];
  const stream = createAgentEventStream(
    new Request("http://localhost/events", { signal: abortController.signal }),
    "session-id",
    startup.promise,
    { onAgentEnd: (sessionId) => { agentEndSessionIds.push(sessionId); } },
  );
  const reader = stream.getReader();
  await readWithin(reader);
  ...
  listener({ type: "message_update", message: {}, assistantMessageEvent: { type: "text_delta", delta: "x" } });
  listener({ type: "agent_end" });
  listener({ type: "agent_end" });

  assert.deepEqual(agentEndSessionIds, ["session-id", "session-id"]);
  abortController.abort();
});
