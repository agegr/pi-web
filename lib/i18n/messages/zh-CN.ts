...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "models.headersHelp": "转发时用配置值覆盖同名请求头（不区分大小写）。值留空的行不会参与覆盖，仅作为待填写的占位。",
    "models.providerHeadersHelp": "添加到该 provider 的每个请求，转发时覆盖同名请求头（不区分大小写）。值留空的行不会参与覆盖，仅作为待填写的占位。",
    "models.headerNamePlaceholder": "请求头名",
    "models.headerValuePlaceholder": "值",
