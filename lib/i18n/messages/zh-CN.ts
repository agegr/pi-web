...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "settings.index": "会话索引",
    "settings.indexDescription": "为会话搜索、导航和加载构建快速 SQLite 索引。禁用后回退到直接扫描 JSONL 文件。修改后需要重启服务器。",
    "settings.indexEnabled": "启用 SQLite 会话索引",
    "settings.retry": "重试",
    "settings.retryDescription": "当模型返回错误时，Pi 会自动以指数退避方式重试请求。禁用后将直接显示错误。",
    "settings.retryEnabled": "错误自动重试",
    "settings.retryMaxRetries": "最大重试次数",
    "settings.retryMaxRetriesDescription": "最大自动重试次数。默认值：3。",
    "settings.retryBaseDelay": "基础延迟（毫秒）",
    "settings.retryBaseDelayDescription": "首次重试前的初始延迟。每次重试后延迟翻倍。默认值：2000ms。",
    "settings.minimap": "聊天小地图",
    "settings.minimapDescription": "配置显示消息位置的聊天小地图。",
    "settings.minimapMaxNodes": "最大节点数（0 = 全部）",
    "settings.minimapMaxNodesHint": "0 显示所有消息。更高的数值限制显示的节点数量。内容超出时垂直滚动。",
