...
test("normalizes OpenCode Go windows into used percentages mapped from the API percent", () => {
  const report = normalizeProviderUsagePayload("opencode-go", {
    usage: {
      rolling: { status: "ok", percent: 1, resetsAt: "2026-09-16T07:15:35.134Z" },
      weekly: { status: "ok", percent: 17, resetsAt: "2026-09-21T00:00:00.134Z" },
      monthly: { status: "ok", percent: 34, resetsAt: "2026-10-10T11:09:48.134Z" },
    },
  }, 123);
  assert.deepEqual(report.buckets.map((bucket) => [bucket.label, bucket.used, bucket.remaining, bucket.unit, bucket.resetsAt]), [
    ["Rolling 5h", 1, 99, "percent", 1789542935],
    ["Weekly", 17, 83, "percent", 1789948800],
    ["Monthly", 34, 66, "percent", 1791630588],
  ]);
  assert.equal(report.providerName, "OpenCode Go");
});

test("records a non-ok OpenCode Go window status as a note", () => {
  const report = normalizeProviderUsagePayload("opencode-go", {
    usage: { rolling: { status: "rate-limited", percent: 100, resetsAt: "2026-09-16T07:15:35.134Z" } },
  }, 123);
  assert.equal(report.buckets.length, 1);
  assert.deepEqual(report.notes, ["Rolling 5h: rate-limited"]);
});

test("rejects an OpenCode Go payload without usage windows", () => {
  assert.throws(() => normalizeProviderUsagePayload("opencode-go", { usage: {} }, 123));
});
