...
test("keeps pi-web thread metadata nodes in otherwise linear branches", () => {
  const thread = node({
    type: "custom",
    id: "thread",
    parentId: "a1",
    ...
    data: { version: 1, sourceEntryId: "a1", hostLeafId: "main", selectedMarkdown: "topic" },
  }, [node(msg("u2", "user", "question"), [node(msg("a2", "assistant", "answer"))])]);
  const [projected] = projectTreeForResponse([node(msg("a1", "assistant", "source"), [thread])]);

  assert.equal(projected.children[0].entry.id, "thread");
  assert.equal(projected.children[0].entry.customType, "pi-web.thread");
  assert.equal(projected.children[0].children[0].entry.id, "a2");
  assert.deepEqual(projected.children[0].children[0].compressedEntryIds, ["u2"]);
});
...
test("keeps a thread's parent so the source stays addressable", () => {
  const thread = node({
    type: "custom",
    id: "thread",
    parentId: "a1",
    timestamp: "t",
    customType: "pi-web.thread",
    data: { version: 1, sourceEntryId: "a1", hostLeafId: "a1", selectedMarkdown: "topic" },
  }, [node(msg("u2", "user", "question"))]);
  const [projected] = projectTreeForResponse([node(msg("u1", "user", "ask"), [node(msg("a1", "assistant", "answer"), [thread])])]);

  const source = projected.children[0];
  assert.equal(source.entry.id, "a1");
  assert.equal(source.compressedEntryIds, undefined);
  assert.equal(source.children[0].entry.id, "thread");
});
