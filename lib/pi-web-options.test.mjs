...
test("network exposure warning reflects that authentication is enabled", async () => {
  const source = await readFile(new URL("../bin/pi-web.js", import.meta.url), "utf8");
  assert.match(source, /Authentication is enabled/);
  assert.doesNotMatch(source, /without authentication/);
});
