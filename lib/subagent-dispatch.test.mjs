...
test("S1: foreground dispatch awaits completion and returns final text", async () => {
  ...
  const childSessionId = controller._runIds[0];
...
test("S1: dispatchId is unique per dispatch", async () => {
  ...
  // Cleanup.
  controller._resolve(controller._runIds[0], "completed", "Done");
  controller._resolve(controller._runIds[1], "completed", "Done");
