...
test("web plugin editor survives the real SDK UI wrapper and retains cancel/replay semantics", async (t) => {
  const { wrapper, ui, events } = setup(t);
  const wrapped = ExtensionRunner.prototype.wrapUIPromptContext.call({ withUIPrompt: (_kind, _title, run) => run() }, ui);
  assert.equal(wrapped.web.version, 1);
  assert.equal(wrapped.web.supportsEditorData, true);
  const data = { language: "markdown" };
  const pending = wrapped.web.editor({ plugin: "demo-editor", view: "preview", title: "Preview", prefill: "# Markdown", data });
  const request = events.at(-1);
  assert.deepEqual(request.web, { plugin: "demo-editor", view: "preview", data });
  assert.equal(request.method, "editor");
  const replay = [];
  wrapper.onEvent((event) => replay.push(event))();
  assert.ok(replay.some((event) => event.id === request.id && event.web.plugin === "demo-editor"));
  await wrapper.send({ type: "extension_ui_response", id: request.id, cancelled: true });
  assert.equal(await pending, undefined);
  assert.equal(wrapper.pendingUiRequests.size, 0);
  await assert.rejects(wrapped.web.editor({ plugin: "../bad", view: "preview", title: "Preview", prefill: "" }), /Invalid/);
  const stopped = wrapped.web.editor({ plugin: "demo-editor", view: "preview", title: "Preview", prefill: "" });
  const rejected = assert.rejects(stopped, { name: "AbortError" });
  await wrapper.send({ type: "abort" });
  await rejected;
});
