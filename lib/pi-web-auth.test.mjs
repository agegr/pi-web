...
test("初始化和改密拒绝空密码、弱密码和超长密码", async () => {
  ...
  for (const password of ["", "short", "a".repeat(129)]) {
    await assert.rejects(() => initializeAuth("setup-token", password), /密码长度无效/);
  }
  await initializeAuth("setup-token", "a-secure-password");
  for (const password of ["", "short", "b".repeat(129)]) {
    await assert.rejects(() => changePassword("a-secure-password", password), /密码长度无效/);
  }
});
