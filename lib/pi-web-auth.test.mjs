...
test("password hashes verify and incorrect passwords fail", async () => {
...
test("initialization and password changes reject empty, weak, and oversized passwords", async () => {
  ...
  for (const password of ["", "short", "a".repeat(129), "aaaaaaaa", "11111111", "\n".repeat(8), "password", "12345678"]) {
    await assert.rejects(() => initializeAuth("setup-token", password), /Invalid password format/);
  }
  await initializeAuth("setup-token", "a-secure-password");
  for (const password of ["", "short", "b".repeat(129), "bbbbbbbb", "qwertyui"]) {
    await assert.rejects(() => changePassword("a-secure-password", password), /Invalid password format/);
  }
});

test("authentication config persists explicit scrypt version and cost parameters", async () => {
...
test("legacy authentication config migrates to versioned scrypt parameters after password mutation", async () => {
...
test("login verification, session creation, and password changes share the auth mutation queue", async () => {
...
test("per-source rate limiting does not lock out normal sources and global protection has a higher independent threshold", async () => {
...
test("the setup token can only be consumed once", async () => {
...
test("changing the password invalidates existing sessions through the generation", async () => {
...
test("revoking all sessions notifies session invalidation subscribers without an auth config", async () => {
...
test("invalidation subscribers for the same session share one expiration timer", async () => {
...
test("rate limiting delays after failures and rejects at the threshold", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
...
test("global rate limiting cannot be bypassed by rotating sources", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
  for (let i = 0; i < 100; i += 1) recordLoginFailure(`source-${i}`);
  assert.equal(checkLoginRateLimit("new-source").allowed, false);
});

test("per-source and global buckets recover after their windows expire", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
...
test("the default config path uses PI_CODING_AGENT_DIR", async () => {
...
test("the config file has 0600 permissions and writes contain no temporary file", async () => {
  const { initializeAuth, getAuthConfigPath } = await loadSubject();
...
test("reading corrupt JSON throws instead of returning an uninitialized state", async () => {
...
test("session TTL is 24 hours", async () => {
...
test("a single session can be revoked", async () => {
...
test("old sessions naturally expire after a process restart", async () => {
...
test("successful initialization revokes existing sessions and advances the generation", async () => {
...
test("new sessions use the persisted generation after a process restart", async () => {
...
test("a process restart after revoking all sessions uses the new persisted generation", async () => {
...
test("a failed revoke-all write does not commit the in-memory generation", async () => {
...
test("concurrent revoke-all calls increment the persisted generation for each call", async () => {
...
test("concurrent initialization allows one request and does not overwrite the config", async () => {
...
test("initialization and revoke-all share the mutation queue and keep memory and disk generations consistent", async () => {
  const { getAuthState, initializeAuth, revokeAllSessions } = await loadSubject();
...
test("a failed initialization mutation does not contaminate subsequent revoke-all state", async () => {
  const { getAuthState, initializeAuth, revokeAllSessions } = await loadSubject();
  const original = process.env.PI_WEB_AUTH_CONFIG_PATH;
...
test("initialization can be retried after target directory write failure following token consumption", async () => {
  const { initializeAuth, getAuthConfigPath } = await loadSubject();
  const original = process.env.PI_WEB_AUTH_CONFIG_PATH;
...
test("initialization rejects a config path that is a directory as corrupt config", async () => {
  ...
  try {
    await assert.rejects(() => initializeAuth("setup-token", "test-password"), /Authentication config path is not a regular file/);
  } finally {
    process.env.PI_WEB_AUTH_CONFIG_PATH = original;
    await rm(directoryPath, { recursive: true, force: true });
  }
...
test("a config status stat error prevents accepting the setup token", async () => {
  const { consumeSetupToken } = await loadSubject();
  const original = process.env.PI_WEB_AUTH_CONFIG_PATH;
...
test("test helpers cannot regenerate or revive the token after setup token consumption", async () => {
  const { consumeSetupToken, getSetupTokenForTests } = await loadSubject();
  assert.equal(consumeSetupToken("setup-token"), true);
  assert.throws(() => getSetupTokenForTests(), /Setup token is unavailable/);
  assert.equal(consumeSetupToken("setup-token"), false);
});

test("getAuthState does not write back an old generation when concurrent with a queued mutation", async () => {
  const { getAuthState, initializeAuth, revokeAllSessions } = await loadSubject();
  await initializeAuth("setup-token", "test-password");
  const revocation = revokeAllSessions();
...
test("invalid authentication config structure throws an explicit error", async () => {
  ...
  for (const config of invalidConfigs) {
    ...
    await assert.rejects(() => getAuthState(), /Invalid authentication config structure/);
    await rm(process.env.PI_WEB_AUTH_CONFIG_PATH, { force: true });
  }
});

test("the setup token is rejected after a config appears", async () => {
  const { consumeSetupToken } = await loadSubject();
  await writeFile(process.env.PI_WEB_AUTH_CONFIG_PATH, "{}");
  assert.equal(consumeSetupToken("setup-token"), false);
});

test("the setup token has high entropy", async () => {
...
test("module loading generates a setup token when no config exists", async () => {
...
test("module loading is silent and does not generate a setup token with a valid config", async () => {
  ...
  assert.equal(result.stdout, "Authentication is already initialized");
  assert.equal(result.stderr, "");
  await rm(directory, { recursive: true, force: true });
});

test("the startup initialization hook prints the token once to server stderr, not HTTP or the config file", async () => {
...
test("startup prints a local recovery path for corrupt config without generating a setup token", async () => {
  ...
  assert.match(result.stderr, /Authentication config is corrupt|back up or repair the config file/);
  assert.match(result.stderr, new RegExp(configPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.doesNotMatch(result.stderr, /setup token: [0-9a-f]{64}/);
  await rm(directory, { recursive: true, force: true });
});

test("an existing config path that is not a regular file is corrupt and rejects the setup token", async () => {
  ...
  try {
    assert.equal(consumeSetupToken("setup-token"), false);
    assert.throws(() => getSetupTokenForTests(), /Authentication config is corrupt/);
  } finally {
    process.env.PI_WEB_AUTH_CONFIG_PATH = original;
    await rm(directoryPath, { recursive: true, force: true });
  }
...
test("a broken symlink config path is corrupt and cannot be overwritten during initialization", async () => {
  ...
  try {
    assert.equal(consumeSetupToken("setup-token"), false);
    assert.throws(() => getSetupTokenForTests(), /Authentication config is corrupt/);
    await assert.rejects(() => initializeAuth("setup-token", "test-password"), /Authentication config is corrupt|Authentication config path is not a regular file/);
...
test("the auth status route returns an uninitialized state", async () => {
...
test("setup token state is stored in globalThis for sharing across Next server bundles", async () => {
  await loadSubject();
  assert.equal(globalThis.__piWebAuthSetupState?.token, "setup-token");
});

test("login session state is stored in globalThis for sharing across Next server bundles", async () => {
  const { authenticateAndCreateSession, initializeAuth } = await loadSubject();
  await initializeAuth("setup-token", "safe-password");
  const token = await authenticateAndCreateSession("safe-password");
  assert.ok(token);
  assert.ok(globalThis.__piWebAuthSessions?.size === 1);
});

test("auth mutation, generation, and invalidation listener state is shared across Next server bundles", async () => {
  await loadSubject();
  const runtime = globalThis.__piWebAuthRuntime;
  assert.ok(runtime);
  assert.ok(runtime.authMutationQueue instanceof Promise);
  assert.ok(runtime.sessionInvalidationListeners instanceof Map);
  assert.ok(runtime.sessionInvalidationTimeouts instanceof Map);
});

test("the default anonymous source uses only the global threshold and five failures do not lock out all clients", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
  for (let i = 0; i < 5; i += 1) recordLoginFailure("anonymous");
  assert.equal(checkLoginRateLimit("anonymous").allowed, true);
  for (let i = 5; i < 100; i += 1) recordLoginFailure("anonymous");
  assert.equal(checkLoginRateLimit("anonymous").allowed, false);
});

test("rate limit checks collect all expired source buckets", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
  const originalNow = Date.now;
  let now = originalNow();
  Date.now = () => now;
  try {
    recordLoginFailure("expired-a");
    recordLoginFailure("expired-b");
    now += 15 * 60 * 1000 + 1;
    checkLoginRateLimit("fresh");
    assert.equal(globalThis.__piWebAuthRuntime?.loginFailures.size, 0);
  } finally {
    Date.now = originalNow;
  }
});

test("concurrent login attempts reserve global capacity before password verification", async () => {
  const { beginLoginAttempt, finishLoginAttempt } = await loadSubject();
  const decisions = Array.from({ length: 5 }, () => beginLoginAttempt("anonymous"));
  assert.deepEqual(decisions.map((decision) => decision.allowed), [true, true, true, true, false]);
  for (let index = 0; index < 4; index += 1) finishLoginAttempt("anonymous", false);
  assert.equal(beginLoginAttempt("anonymous").allowed, true);
  finishLoginAttempt("anonymous", false);
});

test("the setup route rejects an invalid token and allows successful initialization", async () => {
  const routes = await loadRoutes();
...
test("setup token errors reuse login rate limiting and successful initialization does not pollute failure counts", async () => {
  const routes = await loadRoutes();
  const auth = await loadSubject();
...
test("the login route returns 401 for an incorrect password and sets a secure cookie on success", async () => {
  const routes = await loadRoutes();
...
test("the login route returns Retry-After for a temporary global rejection", async () => {
  const routes = await loadRoutes();
  const auth = await loadSubject();
  await auth.initializeAuth("setup-token", "safe-password");
...
test("the status route recognizes valid, expired, and logged-out sessions", async () => {
  const routes = await loadRoutes();
  const auth = await loadSubject();
  await auth.initializeAuth("setup-token", "safe-password");
  const token = auth.createSession();
...
test("the password route revokes the old session after a password change and preserves the old password on write failure", async () => {
  const routes = await loadRoutes();
  const auth = await loadSubject();
...
test("all JSON routes reject incorrect Content-Type and oversized bodies", async () => {
  const routes = await loadRoutes();
...
test("logout and password reject oversized bodies without Content-Length before authentication", async () => {
  const routes = await loadRoutes();
...
test("all JSON routes reject invalid media types with parameters", async () => {
  const routes = await loadRoutes();
  for (const route of [routes.setup, routes.login, routes.logout, routes.password]) {
    const response = await route.POST(new Request("http://localhost/api/auth/test", {
      method: "POST",
...
test("the setup route rejects weak passwords server-side with the unified error", async () => {
  ...
  assert.deepEqual(await response.json(), { errorCode: "AUTH_PASSWORD_INVALID", error: "Invalid password format" });
});

test("auth API error responses contain stable error codes and compatible messages", async () => {
  const sourceText = await readFile(new URL("../app/api/auth/login/route.ts", import.meta.url), "utf8");
  assert.match(sourceText, /AUTH_INVALID_PARAMETERS/);
  assert.match(sourceText, /AUTH_LOGIN_RATE_LIMITED/);
  assert.match(sourceText, /AUTH_LOGIN_FAILED/);
});
