...
test("初始化 token 状态保存在 globalThis 以跨 Next server bundle 共享", async () => {
  await loadSubject();
  assert.equal(globalThis.__piWebAuthSetupState?.token, "setup-token");
});

test("登录 session 状态保存在 globalThis 以跨 Next server bundle 共享", async () => {
...
test("认证事务、generation 和失效监听状态在 Next server bundle 间共享", async () => {
  await loadSubject();
...
test("默认匿名来源只使用全局阈值，不会被五次失败锁死所有客户端", async () => {
  const { checkLoginRateLimit, recordLoginFailure } = await loadSubject();
...
test("setup route 服务端拒绝弱密码并返回统一错误", async () => {
  ...
  assert.deepEqual(await response.json(), { errorCode: "AUTH_PASSWORD_INVALID", error: "密码格式无效" });
});

test("认证 API 错误响应包含稳定错误码和兼容文案", async () => {
