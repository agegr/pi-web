...
test("names with the cheapest thinking level the session model accepts", () => {
  ...
  assert.equal(buildSessionTitleAgentOptions(source).initialState.model, source.state.model);
  assert.equal(buildSessionTitleAgentOptions(source).initialState.thinkingLevel, "minimal");
