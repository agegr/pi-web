...
test("temporary title agent preserves the provider-facing prefix", async () => {
  ...
  // A title needs no reasoning budget, so the source level is not inherited.
...
test("names with the configured model and the cheapest thinking level it accepts", async () => {
  const { resolveTitleThinkingLevel } = await jiti.import("./session-title.ts");
  const source = {
    state: { systemPrompt: "cached system prompt", model: { provider: "test", id: "session-model", reasoning: true }, thinkingLevel: "high", tools: [], messages: [] },
    convertToLlm: (value) => value,
    transformContext: async (value) => value,
    streamFunction: () => {},
    steeringMode: "one-at-a-time",
    followUpMode: "one-at-a-time",
    sessionId: "source-session-id",
  ...
  assert.equal(buildSessionTitleAgentOptions(source, { model: titleModel }).initialState.model, titleModel);
