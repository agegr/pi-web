...
test("CLI SHA-256 global/project content identities never compare against Git SHA-1 trees", async () => {
  for (const scope of ["global", "project"]) {
    const current="b".repeat(64);
    const result=await checkSkillUpdate(install({scope,versionHash:current}),{fetcher:async()=>{throw new Error("Must not compare tree/snapshot identities");},resolveContentHashes:async()=>["a".repeat(64),current]});
    assert.equal(result.state,"up-to-date");assert.equal(result.latestVersion,current);
    const changed=await checkSkillUpdate(install({scope,versionHash:current}),{resolveContentHashes:async()=>["c".repeat(64)]});assert.equal(changed.state,"update-available");
  }
});
