...
test("recording never touches the original session file", async () => {
  const dir = makeDir();
  try {
    // A session JSONL sitting next to the trajectories dir must stay intact.
    const sessionFile = join(dir, "session.jsonl");
    const original = "{\"type\":\"session\"}\n";
    writeFileSync(sessionFile, original);
    const recorder = makeRecorder(dir);
    await recorder.start();
    recorder.onAgentEvent({ type: "turn_start", turnIndex: 0, timestamp: 1 });
    recorder.onAgentEvent({ type: "tool_execution_start", toolCallId: "tc", toolName: "read", args: {} });
    await recorder.close();
    assert.equal(readFileSync(sessionFile, "utf8"), original);
    assert.ok(readdirSync(dir).includes("trajectories"));
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("records turns without turnIndex and stamps the current turn on later events", async () => {
  const dir = makeDir();
  try {
    const recorder = makeRecorder(dir);
    await recorder.start();
    recorder.onAgentEvent({ type: "turn_start" });
    const requestId = recorder.startRequest({ id: "m" }, {}, {});
    recorder.onAgentEvent({ type: "tool_execution_start", toolCallId: "tc", toolName: "read", args: {} });
    recorder.onAgentEvent({ type: "tool_execution_end", toolCallId: "tc", toolName: "read", isError: false });
    recorder.finishRequest(requestId, "complete", {});
    recorder.onAgentEvent({ type: "turn_end", message: {}, toolResults: [] });
    recorder.onAgentEvent({ type: "turn_start" });
    recorder.onAgentEvent({ type: "turn_end", message: {}, toolResults: [] });
    await recorder.flush();
    const result = await readTrajectoryFile(dir, "session-1");
    const turnStarts = result.records.filter((r) => r.kind === "turn_start");
    const turnEnds = result.records.filter((r) => r.kind === "turn_end");
    const request = result.records.find((r) => r.kind === "request_start");
    const tool = result.records.find((r) => r.kind === "tool_start");
    assert.deepEqual(turnStarts.map((r) => r.turnId), ["turn-0", "turn-1"]);
    assert.deepEqual(turnEnds.map((r) => r.turnId), ["turn-0", "turn-1"]);
    assert.equal(request.turnId, "turn-0");
    assert.equal(tool.turnId, "turn-0");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
