...
const { sliceActiveBranch, sliceBranchFromEntry, buildSessionContext } = await jiti.import("./session-reader.ts");
...
// --- Jump windows (conversation search results) ---------------------------
// A jump must stay contiguous with the leaf, because the client only ever pages
// upward. So reaching an old entry means shipping everything after it.

test("sliceBranchFromEntry returns the target plus lead-in through the leaf", () => {
  const entries = linearChain(100);
  const sliced = sliceBranchFromEntry(entries, "e99", "e90", 3);
  assert.equal(sliced[0].id, "e87");
  assert.equal(sliced[sliced.length - 1].id, "e99");
  assert.ok(sliced.some((e) => e.id === "e90"));
});

test("sliceBranchFromEntry clamps the lead-in at the root", () => {
  const entries = linearChain(5);
  const sliced = sliceBranchFromEntry(entries, "e4", "e1", 10);
  assert.deepEqual(sliced.map((e) => e.id), ["e0", "e1", "e2", "e3", "e4"]);
});

test("sliceBranchFromEntry refuses unknown entries, other branches, and far targets", () => {
  const entries = linearChain(50);
  assert.equal(sliceBranchFromEntry(entries, "e49", "missing", 3), null);
  // e10 is not an ancestor of the e5 leaf.
  assert.equal(sliceBranchFromEntry(entries, "e5", "e10", 3), null);
  // Farther from the leaf than the cap allows.
  assert.equal(sliceBranchFromEntry(entries, "e49", "e0", 3, 10), null);
});

test("buildSessionContext with fromEntryId returns the jump window instead of the tail", () => {
  const entries = linearChain(300);
  const ctx = buildSessionContext(entries, "e299", { tail: 50, fromEntryId: "e100", fromLead: 2 });
  assert.equal(ctx.entryIds[0], "e98");
  assert.equal(ctx.entryIds[ctx.entryIds.length - 1], "e299");
  assert.ok(ctx.entryIds.includes("e100"));
  assert.equal(ctx.hasMore, true);
});

test("buildSessionContext falls back to the tail page when the jump is refused", () => {
  const entries = linearChain(300);
  const ctx = buildSessionContext(entries, "e299", { tail: 50, fromEntryId: "e0", fromMaxEntries: 10 });
  assert.deepEqual(ctx.entryIds[0], "e250");
  assert.ok(!ctx.entryIds.includes("e0"));
});

test("a jump window over a deep chain stays iterative", () => {
  const entries = linearChain(5000);
  const sliced = sliceBranchFromEntry(entries, "e4999", "e3200", 6, 2000);
  assert.equal(sliced[sliced.length - 1].id, "e4999");
  assert.equal(sliced[0].id, "e3194");
});
