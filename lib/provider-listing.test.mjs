...
test("Cursor subscription login is labeled and marked as poll-based", () => {
  const providers = [
    provider({ id: "cursor", name: "Cursor", hasApiKeyLogin: false, oauthName: "Cursor (Pro/Ultra/Teams)" }),
  ];
  const oauth = buildOAuthProviderList(providers);
  assert.equal(oauth.length, 1);
  assert.equal(oauth[0].name, "Cursor (Pro/Ultra/Teams)");
  assert.equal(oauth[0].loginMode, "poll");
