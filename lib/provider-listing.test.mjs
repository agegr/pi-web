...
const { buildApiKeyProviderList, buildOAuthProviderList, canRemoveCredential } = await loadSubject();
...
test("a delete route only removes the credential type it owns", () => {
  // The bug this guards: after switching to OAuth, "Disconnect" on a stale
  // API-key row must not delete the fresh OAuth credential (and vice versa).
  assert.equal(canRemoveCredential("oauth", "api_key"), false);
  assert.equal(canRemoveCredential("api_key", "oauth"), false);

  assert.equal(canRemoveCredential("api_key", "api_key"), true);
  assert.equal(canRemoveCredential("oauth", "oauth"), true);
  // Nothing stored (e.g. an env-provided key): removal is a harmless no-op.
  assert.equal(canRemoveCredential(undefined, "api_key"), true);
  assert.equal(canRemoveCredential(undefined, "oauth"), true);
});
