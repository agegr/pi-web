...
export type GlobalShortcut = "abort" | "newSession" | "toggleSessionSearch" | null;
 ...
 */
export function formatShortcutHint(key: string, platform: string): string {
 ...
 *
 * Cmd/Ctrl+J and Cmd/Ctrl+K are ignored while the built-in terminal owns the
...
export function matchGlobalShortcut(
  ...
  const { hasAbortHandler, hasSessionSearchHandler, activeCwd } = options;
