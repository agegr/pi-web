...
test("encodeFilePathForApi encodes the POSIX root as a decodable %2F segment", async () => {
  const { encodeFilePathForApi } = await loadSubject();
  const { filePathFromApiSegments } = await (await import("jiti")).createJiti(import.meta.url).import("./paths.ts");
  // A bare root has no non-empty segments, so it must ride inside one
  // percent-encoded segment that the route decodes back to "/".
  assert.equal(encodeFilePathForApi("/"), "%2F");
  // Next URL-decodes route params, so the handler receives the decoded
  // segment "/" and resolves it back to the filesystem root.
  assert.equal(decodeURIComponent("%2F"), "/");
  assert.equal(filePathFromApiSegments(["/"]), "/");
});
