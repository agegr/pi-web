...
test("streams write arguments from toolcall_start/delta and finalizes on end", () => {
  let state = snapshot(INITIAL_STREAMING_STATE, assistant());
  ...
  state = delta(state, { type: "toolcall_delta", contentIndex: 0, delta: '{"path":"/tmp/a.md","content":"Hel' });
  assert.equal(state.streamingMessage.content[0].toolName, "write");
  assert.equal(state.streamingMessage.content[0].input.path, "/tmp/a.md");
  assert.equal(state.streamingMessage.content[0].input.content, "Hel");

  state = delta(state, { type: "toolcall_delta", contentIndex: 0, delta: 'lo"}' });
  assert.equal(state.streamingMessage.content[0].input.content, "Hello");
  ...
  state = delta(state, {
    ...
    toolCall: {
      ...
      id: "call-1",
      name: "write",
      arguments: { path: "/tmp/a.md", content: "Hello" },
    },
  });
  assert.deepEqual(state.streamingMessage.content, [{
    type: "toolCall",
