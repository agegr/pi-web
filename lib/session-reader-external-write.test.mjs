...
test("resolveSessionIdByPath maps an external session file to its id", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-resolve-by-path-"));
  ...
  const { resolveSessionIdByPath, invalidateSessionListCache } = await jiti.import("./session-reader.ts");
  const { disposeSessionWatcher } = await jiti.import("./session-watcher.ts");
  t.after(() => {
  ...
  const file = join(projectDir, "2026-01-01T00-00-00-000Z_55555555-5555-4555-8555-555555555555.jsonl");
  writeFileSync(file, `${JSON.stringify({ ...header, id: "55555555-5555-4555-8555-555555555555" })}\n`);

  // Header read: the ring's session ids resolve without a full catalogue scan.
  assert.equal(await resolveSessionIdByPath(file), "55555555-5555-4555-8555-555555555555");
  // Resolution is cached: a second call hits the path->id cache.
  assert.equal(await resolveSessionIdByPath(file), "55555555-5555-4555-8555-555555555555");

  // Non-session paths never resolve and never throw.
  assert.equal(await resolveSessionIdByPath(join(agentDir, "sessions", "encoded-cwd", "notes.txt")), undefined);
  assert.equal(await resolveSessionIdByPath(join(projectDir, "missing.jsonl")), undefined);
});
