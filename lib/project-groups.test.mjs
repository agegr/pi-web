...
test("legacy project keys include every raw spelling for one stable identity", () => {
  ...
  // attachSessionProjectInfo can share the representative projectRoot while
  // preserving each session's raw pre-upgrade cwd spelling.
  second.projectRoot = first.projectRoot;

  assert.deepEqual(
    getLegacyProjectKeys([first, second, other], first.projectKey),
    [first.projectRoot, second.cwd],
  );
});
