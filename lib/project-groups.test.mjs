...
test("worktree filtering normalizes browser-visible path variants", () => {
  ...
  const windowsLinked = {
    ...linked,
    id: "windows-linked",
    cwd: "C:\\Repos\\Project-Worktrees\\Feature",
  };

  assert.deepEqual(
    sessionsForWorktree([main, linked], "/repos/project-worktrees/feature").map((item) => item.id),
    ["linked"],
  );
  assert.equal(sameWorkspacePath(windowsLinked.cwd, "c:/repos/project-worktrees/feature/"), true);
  assert.equal(sameWorkspacePath("C:\\", "c:/"), true);
  assert.equal(sameWorkspacePath("/repos/Project", "/repos/project"), false);
  assert.equal(sameWorkspacePath("//Repos/Project", "//repos/project"), false);
  assert.equal(sameWorkspacePath(null, "/repos/project"), false);
});
