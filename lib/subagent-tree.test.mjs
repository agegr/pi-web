...
const { buildSubagentTree, findOwnedSubagent } = await jiti.import("./subagent-tree.ts");
...
test("durable task prefers the live label, then firstMessage, before the session name", () => {
  const root = session("root", "Main task");
  const child = session("child", "subagent-worker-317e1ca0-1", "root");
  const runs = live("317e1ca0", 1, { label: "Inspect RPC" });
...
test("durable task skips a forked firstMessage cloned from the root or parent", () => {
  ...
  const runs = live("317e1ca0", 1, { label: "PPTX to HTML" });
...
test("exact runId/index live state overrides lifecycle, activity, and timing only", () => {
  const root = session("root", "Main task");
  const child = session("child", "subagent-worker-317e1ca0-1", "root", { created: "2026-01-01T00:01:00.000Z" });
  const runs = live("317e1ca0", 1, { agent: "worker", state: "running", activityState: "active_long_running", currentTool: "bash", startedAt: 1000, updatedAt: 1100 });
...
test("nested live entries attach under the matching durable parent", () => {
  ...
  const runs = {
    version: 1,
    entries: [
      { runId: "317e1ca0", index: 1, agent: "worker", state: "running", startedAt: 1000, updatedAt: 1100 },
      { runId: "12ab34cd", index: 0, parentRunId: "317e1ca0", parentIndex: 1, agent: "tester", state: "running", startedAt: 1050, updatedAt: 1080 },
    ],
    total: 2,
    omitted: 0,
  };
...
test("capabilities derive from lifecycle state", () => {
  ...
  const runs = {
    version: 1,
    entries: [
      { runId: "11111111", index: 1, agent: "w", state: "queued", startedAt: 1000, updatedAt: 1000 },
      { runId: "22222222", index: 1, agent: "w", state: "running", startedAt: 1000, updatedAt: 1000 },
      { runId: "33333333", index: 1, agent: "w", state: "paused", startedAt: 1000, updatedAt: 1000 },
      { runId: "44444444", index: 1, agent: "w", state: "complete", startedAt: 1000, updatedAt: 1000 },
    ],
    total: 4,
    omitted: 0,
  };
...
test("needs_attention maps from the package activity state", () => {
  const root = session("root", "Main task");
  const child = session("child", "subagent-worker-317e1ca0-1", "root");
  const runs = live("317e1ca0", 1, { state: "running", activityState: "needs_attention", startedAt: 1000, updatedAt: 1100 });
  const tree = buildSubagentTree({ rootId: "root", sessions: [root, child], runs, rpcAvailable: true, polledAt: 1500 });
...
test("elapsedMs only applies to live non-terminal entries", () => {
  ...
  const runs = {
    version: 1,
    entries: [
      { runId: "11111111", index: 1, agent: "w", state: "running", startedAt: 1000, updatedAt: 1100 },
      { runId: "22222222", index: 1, agent: "w", state: "complete", startedAt: 500, updatedAt: 900 },
    ],
    total: 2,
    omitted: 0,
  };
...
test("sibling order uses durable creation time, then live start, then address", () => {
  ...
  const runs = {
    version: 1,
    entries: [
      { runId: "33333333", index: 1, agent: "w", state: "running", startedAt: 500, updatedAt: 600 },
      { runId: "22222222", index: 1, agent: "w", state: "running", startedAt: 400, updatedAt: 600 },
...
test("rebuilding without live data keeps the complete durable tree", () => {
  ...
  const liveTree = buildSubagentTree({ rootId: "root", sessions: [root, child, grand], runs: live("317e1ca0", 1), rpcAvailable: true, polledAt: 2000 });
