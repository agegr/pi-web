...
test("distinguishes files from folders and pi slash commands", async () => {
  const { looksLikeLocalFileReference } = await loadSubject();

  assert.equal(looksLikeLocalFileReference("components/MarkdownBody.tsx:42"), true);
  assert.equal(looksLikeLocalFileReference("C:\\\\work\\\\project\\\\README"), true);
  assert.equal(looksLikeLocalFileReference("Dockerfile"), true);
  assert.equal(looksLikeLocalFileReference(".gitignore"), true);
  assert.equal(looksLikeLocalFileReference("components/"), false);
  assert.equal(looksLikeLocalFileReference("docs/reference"), false);
  assert.equal(looksLikeLocalFileReference("/model"), false);
  assert.equal(looksLikeLocalFileReference("/compact"), false);
});

test("does not treat provider model ids as files", async () => {
  const { looksLikeLocalFileReference } = await loadSubject();

  assert.equal(looksLikeLocalFileReference("openai-codex/gpt-5.6-sol"), false);
  assert.equal(looksLikeLocalFileReference("openai-codex/gpt-5.6-terra"), false);
  assert.equal(looksLikeLocalFileReference("openai-codex/gpt-5.6-luna"), false);
  assert.equal(looksLikeLocalFileReference("archives/release.7z"), true);
});
