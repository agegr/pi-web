...
test("resolves inline-code spans that are only a path", async () => {
  const { resolveInlineCodePath } = await loadSubject();

  assert.equal(resolveInlineCodePath("F:/blendermagic/b"), "F:/blendermagic/b");
  assert.equal(resolveInlineCodePath("C:\\Program Files\\app\\out.txt"), "C:/Program Files/app/out.txt");
  assert.equal(resolveInlineCodePath("/usr/local/bin/tool"), "/usr/local/bin/tool");
  assert.equal(resolveInlineCodePath("b/note.txt", "F:/blendermagic"), "F:/blendermagic/b/note.txt");
  assert.equal(resolveInlineCodePath("src/lib/file-links.ts", "F:/blendermagic"), "F:/blendermagic/src/lib/file-links.ts");
  assert.equal(resolveInlineCodePath("F:/blendermagic/b/"), "F:/blendermagic/b");
});

test("leaves prose inline code alone", async () => {
  const { resolveInlineCodePath } = await loadSubject();

  assert.equal(resolveInlineCodePath("npm test"), null);
  assert.equal(resolveInlineCodePath("mkdir -p b"), null);
  assert.equal(resolveInlineCodePath("--include=dev"), null);
  assert.equal(resolveInlineCodePath("application/json"), null);
  assert.equal(resolveInlineCodePath("read/write"), null);
  assert.equal(resolveInlineCodePath("Node.js"), null);
  assert.equal(resolveInlineCodePath("v0.9.2"), null);
  assert.equal(resolveInlineCodePath("https://example.com/a.txt"), null);
  assert.equal(resolveInlineCodePath("/api/files/foo.txt?type=read"), null);
  assert.equal(resolveInlineCodePath(""), null);
});

test("keeps relative inline-code paths inside their base directory", async () => {
  const { resolveInlineCodePath } = await loadSubject();

  assert.equal(resolveInlineCodePath("b/note.txt"), null);
  assert.equal(resolveInlineCodePath("../../etc/passwd", "/home/me/project"), null);
});
