...
const { resolveVisibleModels, filterModelScopeByDisabledProviders, selectInitialModelScope } = await loadSubject();
...
test("filters disabled providers from visible and scoped models", async () => {
  const scope = await resolveVisibleModels(runtime, ["anthropic/*:high", "acme-gateway/*:low"]);
  const filtered = filterModelScopeByDisabledProviders(scope, new Set(["anthropic"]));

  assert.deepEqual(refs(filtered).sort(), [
    "acme-gateway/claude-opus-4-8",
    "acme-gateway/claude-sonnet-5",
  ]);
  assert.equal(filtered.scopedModels.length, 2);
  assert.deepEqual(filtered.thinkingLevelPins, {
    "acme-gateway/claude-opus-4-8": "low",
    "acme-gateway/claude-sonnet-5": "low",
  });
  assert.equal(
    selectInitialModelScope(filtered, { defaultModel: { provider: "anthropic", modelId: "claude-opus-5" } }).model.provider,
    "acme-gateway",
  );
});
