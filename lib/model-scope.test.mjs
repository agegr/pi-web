...
test("keeps thinking-level suffixes out of the matched reference", async () => {
...
test("falls back to all available models when the resolver throws", async () => {
  const brokenRuntime = {
    getAvailable: async () => {
      const models = [...MODELS];
      // A non-iterable clone breaks the resolver's internal spread.
      Object.defineProperty(models, Symbol.iterator, { value: undefined });
      return models;
    },
  };

  const result = await resolveVisibleModels(brokenRuntime, ["anthropic/*"]);
  assert.equal(result.visible.length, MODELS.length);
  assert.deepEqual(result.warnings, []);
});
