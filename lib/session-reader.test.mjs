...
const scannerModule = await jiti.import("./session-list-scanner.ts");
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  ...
  const originalScan = scannerModule.listSessionsIncremental;
  const originalOpen = SessionManager.open;
  let fullOpens = 0;
  scannerModule.listSessionsIncremental = async () => [{
    path: filePath,
    id: childId,
    cwd: dir,
    name: undefined,
  ...
  resetSessionListState();
  t.after(() => {
    scannerModule.listSessionsIncremental = originalScan;
