...
test("preserves active transcript messages before a compaction boundary", () => {
  const entries = [
    userEntry("u1", null, "request"),
    assistantEntry("a1", "u1", "working"),
    {
      type: "compaction",
      id: "cmp",
      parentId: "a1",
  ...
  assert.deepEqual(context.entryIds, ["u1", "a1", "cmp"]);
});

test("renders the full active transcript with aligned entry IDs", () => {
  const entries = [
  ...
  assert.deepEqual(
    context.messages.map((message) => [message.role, message.customType]),
    [
      ["user", undefined],
      ["assistant", undefined],
      ["user", undefined],
...
test("preserves every compaction marker on the active path", () => {
  const entries = [
  ...
  assert.deepEqual(
    context.messages.filter((message) => message.role === "custom").map((message) => message.content),
    ["first summary", "latest summary"],
  );
...
test("preserves valid epoch timestamps on synthetic UI messages", () => {
  ...
  const compaction = context.messages.find((message) => message.role === "custom");
  assert.equal(compaction?.customType, "compaction");
  assert.equal(compaction?.timestamp, 0);
});
