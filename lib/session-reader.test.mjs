...
test("preserves timeToFirstTokenMs and derives endedAt from the entry timestamp", () => {
  const generationStart = Date.parse("2026-01-01T00:00:03.000Z");
  const persistedAt = Date.parse("2026-01-01T00:00:05.000Z");
  const entries = [
    userEntry("u1", null, "prompt", "2026-01-01T00:00:01.000Z"),
    {
      type: "message",
      id: "a1",
      parentId: "u1",
  ...
  // TTFT survives through normalizeToolCalls' spread (the value pi persisted).
  assert.equal(assistant.timeToFirstTokenMs, 10494);
  // endedAt is re-derived from the outer entry timestamp on reload.
  assert.equal(assistant.endedAt, persistedAt);
  // The message's own timestamp stays the generation-start marker.
  assert.equal(assistant.timestamp, generationStart);
});
