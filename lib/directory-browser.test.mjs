...
test("resolves directory enumeration at runtime instead of during output tracing", async () => {
  const source = await readFile(new URL("./directory-browser.ts", import.meta.url), "utf8");

  assert.match(source, /process\.getBuiltinModule\("node:fs\/promises"\)/);
  assert.match(source, /getOwnPropertyDescriptor\(fs, "opendir"\)/);
  assert.doesNotMatch(source, /import \{[^}]*opendir/);
});
