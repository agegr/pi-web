...
test("request security rejects untrusted hosts with the legacy response matrix", async () => {
  const { getRequestSecurityRejection } = await jiti.import("../src/request-security.ts");
...
test("request security allows HTTPS Origin when Host includes the default https port", async () => {
  const { getRequestSecurityRejection } = await jiti.import("../src/request-security.ts");
  process.env.PI_WEB_ALLOWED_HOSTS = "pi.example.com";
  try {
    const response = getRequestSecurityRejection(new Request("http://localhost:30141/api/agent/new", {
      method: "POST",
      headers: {
...
test("request security allows a configured hostname when reverse-proxy ports differ", async () => {
  const { getRequestSecurityRejection } = await jiti.import("../src/request-security.ts");
  process.env.PI_WEB_ALLOWED_HOSTS = "pi.example.com";
  try {
    const response = getRequestSecurityRejection(new Request("http://localhost:30141/api/agent/new", {
