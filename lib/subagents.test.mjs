...
const {
  ...
  externalSubagentRunFromEntry,
  matchExternalSubagentRun,
...
test("reads third-party subagent runs from parent entries and matches child names", () => {
  // Shapes taken from a real session: the record entry and the spawn tool result.
  const record = externalSubagentRunFromEntry({
    type: "custom",
    customType: "subagents:record",
    data: {
      id: "35a50c91-c27b-473",
  ...
  const spawned = externalSubagentRunFromEntry({
    type: "message",
    message: {
      role: "toolResult",
      details: { subagentType: "Explore", description: "Inspect parser", status: "background", agentId: "aa11bb22-cc33" },
    },
  });
  ...
  // Only entries that actually carry a run identity qualify.
  assert.equal(externalSubagentRunFromEntry({ type: "message", message: { role: "user", details: { agentId: "x" } } }), null);
  assert.equal(externalSubagentRunFromEntry({ type: "message", message: { role: "toolResult", details: {} } }), null);
  assert.equal(externalSubagentRunFromEntry({ type: "custom", customType: "subagents:record", data: { type: "Explore" } }), null);
  assert.equal(externalSubagentRunFromEntry({ type: "custom", customType: "other", data: { id: "x" } }), null);
  assert.equal(externalSubagentRunFromEntry(null), null);

  const runs = [record, spawned];
  assert.equal(matchExternalSubagentRun(runs, "general-purpose#35a50c91"), record);
  assert.equal(matchExternalSubagentRun(runs, "Explore#aa11bb22"), spawned);
  // A name without a run id, an unrecorded id, or no runs at all is not a subagent.
  assert.equal(matchExternalSubagentRun(runs, "general-purpose"), undefined);
  assert.equal(matchExternalSubagentRun(runs, "general-purpose#deadbeef"), undefined);
  assert.equal(matchExternalSubagentRun(runs, "general-purpose#35a50c9"), undefined);
  assert.equal(matchExternalSubagentRun([], "general-purpose#35a50c91"), undefined);
  assert.equal(matchExternalSubagentRun(runs, undefined), undefined);
});
