...
test("a saved skill or extension scope survives the round trip", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-"));
  try {
    // A new profile: the list the caller passed must reach the file, not a boolean
    // that reads back as "every discovered skill".
    const saved = saveProjectSubagentProfile(cwd, profile({
      loadSkills: true,
      skills: ["code-review", " code-review ", "testing"],
      loadExtensions: true,
      extensionScope: ["CodeGraph", "codegraph"],
    }));
    ...
    assert.deepEqual(loaded.extensionScope, ["codegraph"]);
    assert.match(source, /skills:/);
    assert.doesNotMatch(source, /^skills: true$/m);

    // Editing a scope that is already in the file must win over what was stored,
    // and an explicit empty scope must not be widened back to "unbounded".
    saveProjectSubagentProfile(cwd, { ...loaded, skills: ["testing"], extensionScope: [] });
    assert.deepEqual(resolveSubagentProfile(cwd, "test-agent").skills, ["testing"]);
    assert.deepEqual(resolveSubagentProfile(cwd, "test-agent").extensionScope, []);

    // No scope keeps the boolean switch, and a hand-authored whitelist survives.
    ...
    // A hand-authored whitelist this app did not change stays exactly as authored,
    // scalar spelling included.
    await writeFile(
      join(cwd, ".pi", "agents", "foreign.md"),
      "---\ndescription: d\nload_extensions: true\nextensions: pi-advisor-flow\n---\nBody\n",
    );
    saveProjectSubagentProfile(cwd, resolveSubagentProfile(cwd, "foreign"));
    const foreignSource = await readFile(join(cwd, ".pi", "agents", "foreign.md"), "utf8");
    assert.match(foreignSource, /^extensions: pi-advisor-flow$/m);
    assert.deepEqual(resolveSubagentProfile(cwd, "foreign").extensionScope, ["pi-advisor-flow"]);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});

test("a malformed persisted scope fails closed instead of widening", () => {
  ...
  // Absent means the snapshot predates the field; present-but-invalid must not
  // read as "unbounded", which is what the child was scoped away from.
  ...
  for (const extensionScope of ["codegraph", [7], {}]) {
    assert.throws(() => readSubagentSessionResources(snapshot({ extensionScope })), /Invalid subagent extension scope/);
  }
  // Empty scopes are valid and stay empty.
  assert.deepEqual(readSubagentSessionResources(snapshot({ skills: [], extensionScope: [] })).skills, []);
});
