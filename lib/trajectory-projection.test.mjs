...
test("cursor filters older records without changing stats", () => {
  const records = [
  ...
  assert.deepEqual(projection.records.map((r) => r.id), ["mid", "new"]);
  assert.equal(projection.stats.requests, 2);
  assert.equal(projection.hasOlderRecords, true);
});
