...
test("ledger rows use paired start/end timestamps, not the end record's own clock", () => {
  // Recorder writes endTimestamp = timestamp = now() on the end record, so
  // endTimestamp - timestamp is always 0. Duration must come from pairing.
  const records = [
    record({ sequence: 1, id: "req", kind: "request_start", leafId: "e1", requestId: "req", timestamp: 1000 }),
    record({
      sequence: 2,
      id: "end",
      kind: "request_end",
    ...
    record({ sequence: 3, id: "tool", kind: "tool_start", leafId: "e1", stepId: "tc1", timestamp: 5100 }),
    record({
      sequence: 4,
      id: "tool-end",
      kind: "tool_end",
    ...
    record({ sequence: 5, id: "comp", kind: "compaction_start", leafId: "e1", timestamp: 6000 }),
    record({
      sequence: 6,
      id: "comp-end",
      kind: "compaction_end",
  ...
  const byId = Object.fromEntries(projection.records.map((row) => [row.id, row]));
  assert.equal(byId.req.durationMs, 4000);
  assert.equal(byId.req.endTimestamp, 5000);
  assert.equal(byId.req.status, "complete");
  assert.equal(byId.end.durationMs, 4000);
  assert.equal(byId.tool.durationMs, 800);
  assert.equal(byId["tool-end"].durationMs, 800);
  assert.equal(byId.comp.durationMs, 400);
  assert.equal(byId["comp-end"].durationMs, 400);
});

test("infers turn ids from request groups when the sidecar has no turn records", () => {
  const records = [
    record({ sequence: 1, id: "sess", kind: "session_start", leafId: null, timestamp: 1 }),
    record({ sequence: 2, id: "r1", kind: "request_start", leafId: "e1", requestId: "r1", timestamp: 1000 }),
    record({ sequence: 3, id: "t1", kind: "tool_start", leafId: "e1", stepId: "tc1", timestamp: 2000 }),
    record({ sequence: 4, id: "r2", kind: "request_start", leafId: "e1", requestId: "r2", timestamp: 3000 }),
  ];
  const projection = projectTrajectory(readResult(records), {
    leafId: "leaf",
    detailLevel: "summary",
  ...
  const byId = Object.fromEntries(projection.records.map((row) => [row.id, row]));
  assert.equal(byId.sess.turnId, undefined);
  assert.equal(byId.r1.turnId, "turn-0");
  assert.equal(byId.t1.turnId, "turn-0");
  assert.equal(byId.r2.turnId, "turn-1");
  assert.equal(projection.stats.turns, 2);
  assert.equal(projection.turns[0].id, "turn-0");
  assert.equal(projection.turns[0].requestCount, 1);
  assert.equal(projection.turns[0].toolCount, 1);
});
