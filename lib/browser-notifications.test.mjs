...
async function loadSubject() {
  // Loaded through jiti so the module's own extensionless imports resolve the
  // way the app resolves them (tsconfig moduleResolution: "bundler");
  // bare `import("./browser-notifications.ts")` only works while that file
  // has no value imports (it now value-imports the Capacitor bridge).
  const { createJiti } = await import("jiti");
  return createJiti(import.meta.url).import("./browser-notifications.ts");
}
...
test("background-task terminal notifications fire only when the page is not attended", async () => {
  const { shouldShowBrowserNotification } = await loadSubject();
  // Visible and focused: the in-app panel state is the notice; no browser notification.
  assert.equal(shouldShowBrowserNotification({ visibilityState: "visible", hasFocus: () => true }), false);
  // Hidden or unfocused: one browser notification per terminal transition (dedupe by id
  // lives in hooks/useBackgroundTasks.ts::markTerminalNotified).
  assert.equal(shouldShowBrowserNotification({ visibilityState: "hidden", hasFocus: () => true }), true);
  assert.equal(shouldShowBrowserNotification({ visibilityState: "visible", hasFocus: () => false }), true);
});

test("delivers through the Capacitor LocalNotifications scheduler when the shell bridge is present (pi#31)", async () => {
  const { showBrowserNotification } = await loadSubject();
  const scheduled = [];
  ...
  const delivery = await showBrowserNotification({
    ...
    sessionUrl: "/?session=session-1",
    tag: "session-complete:session-1",
    onClick: () => assert.fail("tap routing is owned by the performed-event listener"),
  }, {
    createWindowNotification: () => {
      throw new Error("unexpected constructor call");
    },
    getServiceWorkerRegistration: async () => {
      throw new Error("unexpected service-worker call");
    },
    scheduleLocalNotification: async (title, options, sessionUrl) => {
      scheduled.push({ title, options, sessionUrl });
      return true;
    },
  });
  ...
  assert.deepEqual(scheduled, [{
    title: "Session complete",
    options: { body: "Task finished.", tag: "session-complete:session-1", renotify: true },
    sessionUrl: "/?session=session-1",
  }]);
});

test("falls back to the web delivery paths when the shell scheduler reports failure (pi#31)", async () => {
  const { showBrowserNotification } = await loadSubject();
  let clicked = false;
  ...
  const delivery = await showBrowserNotification({
    ...
    createWindowNotification: () => notification,
    getServiceWorkerRegistration: null,
    scheduleLocalNotification: async () => false, // denied / plugin unavailable
  });
...
test("a scheduler that throws falls back instead of rejecting (pi#31)", async () => {
  const { showBrowserNotification } = await loadSubject();
  const shown = [];
  ...
  const delivery = await showBrowserNotification({
    ...
    createWindowNotification: () => ({ onclick: null, close: () => {} }),
    getServiceWorkerRegistration: async () => ({
      showNotification: async (title, options) => shown.push({ title, options }),
    }),
    scheduleLocalNotification: async () => {
      throw new Error("plugin bridge gone");
    },
  });
...
test("environments without a shell scheduler are unaffected", async () => {
  const { showBrowserNotification } = await loadSubject();
  const shown = [];
  const delivery = await showBrowserNotification({
    title: "Session complete",
  ...
  assert.equal(delivery, "service-worker");
  assert.equal(shown.length, 1);
  assert.deepEqual(shown[0].options.data, { url: "/?session=session-1" });
});
