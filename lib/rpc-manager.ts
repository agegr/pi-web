...
export class AgentSessionWrapper {
  ...
  /** 快照当前待处理的扩展 UI 请求（插入序）：客户端冷启动/SSE 重放丢失后的
   * 主动恢复兑底（GET /api/agent/[id] state 附带）。含被动 custom overlay。 */
  getPendingUiRequests(): ExtensionUiRequest[] {
    // 入库时均为 as ExtensionUiRequest 的结构化事件（emitCustomUiRender / sendUiRequest），
    // map 声明用宽泛 AgentEvent 只是历史类型借用。
    return [...this.pendingUiRequests.values()] as ExtensionUiRequest[];
  }
