...
test("provider OAuth session invalidation aborts and closes SSE before late OAuth completion", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => loginInteraction.prompt({ type: "manual_code", message: "late code", placeholder: "" }),
      /Login cancelled/,
    );
    loginInteraction.notify({ type: "auth_url", url: "https://example.test/late", instructions: "late" });
    assert.equal(globalThis.__piLoginCallbacks.size, 0);
...
test("provider OAuth does not start when the request is already aborted", async () => {
  const directory = await mkdtemp(join(tmpdir(), "pi-web-provider-oauth-pre-abort-"));
  const previousConfigPath = process.env.PI_WEB_AUTH_CONFIG_PATH;
  const previousCreate = ModelRuntime.create;
...
test("provider OAuth abort during runtime creation never starts login or creates callbacks", async () => {
  const directory = await mkdtemp(join(tmpdir(), "pi-web-provider-oauth-create-abort-"));
  const previousConfigPath = process.env.PI_WEB_AUTH_CONFIG_PATH;
  const previousCreate = ModelRuntime.create;
  ...
  try {
    ...
    const client = new AbortController();
    const { GET } = await jiti.import("../app/api/auth/login/[provider]/route.ts");
    const responsePromise = GET(new Request("http://localhost/api/auth/login/anthropic", {
      signal: client.signal,
      headers: { cookie: `pi_web_session=${sessionToken}` },
    }), { params: Promise.resolve({ provider: "anthropic" }) });
