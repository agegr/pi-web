...
const { resolveSubagentResources } = await jiti.import("./subagent-dispatch.ts");
...
test("G6 source: three-level fallback and auditSessionManager resolution", async () => {
  ...
  // The three-level fallback for persistSession lives in the unified resolver:
  // dispatchPersistSession ?? profilePersistSession ?? true.
  assert.match(dispatchSource, /persistSession = params\.dispatchPersistSession \?\? params\.profilePersistSession \?\? true/);

  // auditSessionManager resolves to parent when persistSession is false.
  assert.match(runtimeSource, /auditSessionManager = persistSession \? sessionManager : parent\.inner\.sessionManager/);

  // Metadata entry uses parent.inner.sessionManager in the else branch.
  assert.match(runtimeSource, /parent\.inner\.sessionManager\.appendCustomEntry\(SUBAGENT_META_TYPE/);

  // Result and status entries use auditSessionManager.
  assert.match(runtimeSource, /auditSessionManager\.appendCustomEntry\(SUBAGENT_RESULT_TYPE/);
  assert.match(runtimeSource, /auditSessionManager\.appendCustomEntry\(SUBAGENT_STATUS_TYPE/);
});

// G6 — Behavioral: profile.persistSession=false without request override → inMemory
// ---------------------------------------------------------------------------
// Proves the three-level fallback (dispatch → profile → true) is live:
// when the profile sets persistSession=false and the request omits it,
// the resolver produces false and start() creates an in-memory session
// (sessionPath empty).

test("G6 behavior: profile.persistSession=false without request override → inMemory", () => {
  // Unit-level: resolver three-level fallback.
  const fromProfile = resolveSubagentResources({
    profileTools: [],
    profilePersistSession: false,
  });
  assert.equal(fromProfile.persistSession, false,
    "profile.persistSession=false must propagate when dispatch omits it");

  const dispatchOverridesProfile = resolveSubagentResources({
    profileTools: [],
    dispatchPersistSession: true,
    profilePersistSession: false,
  });
  assert.equal(dispatchOverridesProfile.persistSession, true,
    "dispatchPersistSession must override profile.persistSession");

  const neitherSet = resolveSubagentResources({
    profileTools: [],
  });
  assert.equal(neitherSet.persistSession, true,
    "when both dispatch and profile omit persistSession, default is true");
});
