...
test("G6 source: three-level fallback and auditSessionManager resolution", async () => {
  ...
  // The three-level fallback for persistSession: resolved through the unified
  // resolution pipeline (resolveSubagentResources) which computes
  // dispatchPersistSession ?? true.
  assert.match(source, /const \{ persistSession \} = plan/);
