...
const { removeStoredCredentialIfType, storeProviderCredential } = await createJiti(
  import.meta.url,
...
test("missing credentials are a successful no-op", async () => {
  await withAuthFile({}, async (authPath) => {
    assert.deepEqual(await removeStoredCredentialIfType("anthropic", "oauth", authPath), {
      status: "not_found",
    });
    assert.deepEqual(await readAuthFile(authPath), {});
  });
