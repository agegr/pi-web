...
test("direct destruction emits session_shutdown before dispose when extensions are present", async () => {
  ...
  assert.deepEqual(calls, [
    ["emit", { type: "session_shutdown", reason: "dispose" }],
    ["dispose"],
    ["destroy"],
  ]);
  assert.equal(wrapper.isAlive(), false);
...
test("idle timer forces shutdown even while the session appears stuck running", async () => {
  ...
  assert.match(idleSource, /this\.inner\.abort\(\)/);
  assert.doesNotMatch(idleSource, /this\.resetIdleTimer\(\);\s*return;/);
});
