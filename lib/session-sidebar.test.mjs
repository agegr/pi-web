...
test("buildSessionTree terminates on a direct parent cycle (characterizes current behavior)", async () => {
  const { buildSessionTree } = await subject();
  // a<->b each name the other as parent, and both exist in the set. Each node
  // resolves the other as its ancestor, so both get attached and NEITHER is a
  // root -> roots is empty. The point of this test is that it returns without
  // hanging; the empty result documents (not endorses) the existing behavior.
  const roots = buildSessionTree([
