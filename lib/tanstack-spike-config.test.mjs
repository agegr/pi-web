...
test("keeps Next commands and publication files while adding spike commands", () => {
  ...
  assert.equal(pkg.scripts["dev:tanstack"], "vite dev --configLoader runner --config vite.tanstack.config.ts --host 127.0.0.1 --port 30142");
  assert.equal(pkg.scripts["build:tanstack"], "vite build --configLoader runner --config vite.tanstack.config.ts");
...
test("the config supports explicit standalone and publication output modes", () => {
  ...
  assert.match(viteConfig, /outputMode === "standalone"/);
  assert.match(viteConfig, /traceDeps: outputMode === "standalone" \? EXTERNAL_PACKAGES : \[\]/);
  assert.match(viteConfig, /copyExternalPackages/);
