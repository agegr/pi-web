...
const { buildSubagentPromptPlan, composeSubagentExactPrompt } = await createJiti(import.meta.url).import("./subagent-prompt.ts");
...
test("the exact prompt appends skill text after the profile body", () => {
  assert.equal(composeSubagentExactPrompt("Only these instructions.", ""), "Only these instructions.");
  assert.equal(
    composeSubagentExactPrompt("Only these instructions.", "<skill name=\"a\">A</skill>\n\n<skill name=\"b\">B</skill>"),
