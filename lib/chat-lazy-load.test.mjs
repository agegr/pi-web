...
test("continues paging when render items outnumber source messages", async () => {
  const { VISIBLE_PAGE_SIZE, getNextVisibleCount, getVisibleRenderWindow } = await loadSubject();
  let visibleCount = VISIBLE_PAGE_SIZE;

  visibleCount = getNextVisibleCount(visibleCount);
  assert.deepEqual(getVisibleRenderWindow(120, visibleCount), { startIndex: 120 - 2 * VISIBLE_PAGE_SIZE, hasMore: true });

  visibleCount = getNextVisibleCount(visibleCount);
  assert.deepEqual(getVisibleRenderWindow(120, visibleCount), { startIndex: 120 - 3 * VISIBLE_PAGE_SIZE, hasMore: true });
