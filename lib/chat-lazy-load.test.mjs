...
test("captures a bottom-anchored scroll snapshot with its DOM fingerprint", async () => {
  const { captureScrollAnchor, restoreScrollTop } = await loadSubject();
  // User pinned at the very top of loaded history (pi#16 scenario).
  const snapshot = captureScrollAnchor(3864, 0, "e4950");
  assert.deepEqual(snapshot, { distance: 3864, scrollHeight: 3864, firstEntryId: "e4950" });
  // One 50-message page (~3815px) prepends above: restoring the captured
  // distance keeps the viewport on the same reading position.
  assert.equal(restoreScrollTop(3864 + 3815, snapshot.distance), 3815);
});

test("restores the anchor once a prepend commits to the DOM", async () => {
  const { captureScrollAnchor, shouldRestoreScrollAnchor } = await loadSubject();
  const snapshot = captureScrollAnchor(2000, 350, "e4950");
  // Prepend committed: height grew and the first rendered entry changed.
  assert.equal(shouldRestoreScrollAnchor(snapshot, 2000 + 800, "e4900"), true);
});

test("does not restore before the prepend commits", async () => {
  const { captureScrollAnchor, shouldRestoreScrollAnchor } = await loadSubject();
  const snapshot = captureScrollAnchor(2000, 350, "e4950");
  // messages.length changed but the DOM has not: same height, same first entry.
  assert.equal(shouldRestoreScrollAnchor(snapshot, 2000, "e4950"), false);
});

test("tail growth does not consume the anchor", async () => {
  const { captureScrollAnchor, shouldRestoreScrollAnchor } = await loadSubject();
  const snapshot = captureScrollAnchor(2000, 350, "e4950");
  // Streaming appended below the viewport: height grew, first entry unchanged.
  assert.equal(shouldRestoreScrollAnchor(snapshot, 2000 + 500, "e4950"), false);
});

test("restores on any growth when the first entry is unknown at capture", async () => {
  const { captureScrollAnchor, shouldRestoreScrollAnchor } = await loadSubject();
  const snapshot = captureScrollAnchor(2000, 350, null);
  assert.equal(shouldRestoreScrollAnchor(snapshot, 2000, null), false);
  assert.equal(shouldRestoreScrollAnchor(snapshot, 2500, null), true);
});
