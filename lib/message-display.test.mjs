...
test("hidden extension messages are filtered without changing source messages or entry IDs", async () => {
  const { filterHiddenExtensionMessages } = await loadSubject();
  const messages = [
    { role: "user", content: "prompt" },
    { role: "custom", customType: "context-prune-summary", display: false, content: "hidden" },
    { role: "toolResult", content: [] },
    { role: "custom", customType: "visible", display: true, content: "visible" },
    { role: "custom", customType: "default", content: "default" },
    assistant([{ type: "text", text: "answer" }]),
    { role: "custom", customType: "context-prune-summary", display: false, content: "tail" },
  ];
  const ids = messages.map((_, i) => `entry-${i}`);
  const original = structuredClone(messages);
  const hidden = filterHiddenExtensionMessages(messages, ids, false);
  assert.deepEqual(hidden.entryIds, ["entry-0", "entry-2", "entry-3", "entry-4", "entry-5"]);
  assert.deepEqual(hidden.messages, [messages[0], messages[2], messages[3], messages[4], messages[5]]);
  assert.deepEqual(messages, original);
  assert.equal(ids.length, 7);
  const shown = filterHiddenExtensionMessages(messages, ids, true);
  assert.equal(shown.messages, messages);
  assert.equal(shown.entryIds, ids);
});
