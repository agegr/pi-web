...
test("collects tool result images in turn order and skips non-assistant messages", async () => {
  const { collectTurnToolResultImages } = await loadSubject();
  const first = { type: "image", source: { type: "url", url: "https://example.com/one.png" } };
  const second = { type: "image", source: { type: "url", url: "https://example.com/two.png" } };
  const messages = [
    { role: "user", content: "look" },
    assistant([{ type: "toolCall", toolCallId: "call-1", toolName: "page_screenshot", input: {} }]),
    { role: "toolResult", toolCallId: "call-1", content: [first] },
    assistant([
      { type: "toolCall", toolCallId: "call-2", toolName: "bash", input: {} },
      { type: "toolCall", toolCallId: "call-3", toolName: "page_screenshot", input: {} },
    ]),
    assistant([{ type: "text", text: "done" }]),
  ];
  const toolResults = new Map([
    ["call-1", { role: "toolResult", toolCallId: "call-1", content: [first] }],
    ["call-2", { role: "toolResult", toolCallId: "call-2", content: [{ type: "text", text: "ok" }] }],
    ["call-3", { role: "toolResult", toolCallId: "call-3", isError: true, content: [second] }],
  ]);

  assert.deepEqual(
    collectTurnToolResultImages(messages, 1, 4, toolResults).map((image) => image.source.url),
    ["https://example.com/one.png", "https://example.com/two.png"],
  );
  assert.deepEqual(collectTurnToolResultImages(messages, 4, 4, toolResults), []);
});
