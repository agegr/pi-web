...
test("returns completed provider errors even when the message has no content", async () => {
  ...
  assert.equal(getAssistantErrorMessage(message), "OpenAI API error (403): request forbidden");
...
test("falls back when a provider error has no message", async () => {
  ...
  assert.equal(getAssistantErrorMessage({ ...assistant([]), stopReason: "stop" }), null);
});
