...
test("keeps process text and images in chronological order while hiding thinking and tool calls", async () => {
  const { getProcessContentBlocks } = await loadSubject();
  const directImage = { type: "image", source: { type: "url", url: "https://example.com/direct.png" } };
  const toolImage = { type: "image", source: { type: "url", url: "https://example.com/tool.png" } };
  const message = assistant([
    { type: "thinking", thinking: "inspect first" },
    { type: "text", text: "Visible process text" },
    { type: "toolCall", toolCallId: "call-1", toolName: "page_screenshot", input: {} },
    directImage,
  ]);
  const toolResults = new Map([[
    "call-1",
    { role: "toolResult", toolCallId: "call-1", content: [{ type: "text", text: "hidden output" }, toolImage] },
  ]]);

  const result = getProcessContentBlocks(message, toolResults);

  assert.deepEqual(result, [message.content[1], toolImage, directImage]);
});
