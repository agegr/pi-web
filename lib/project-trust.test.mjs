...
import { getProjectTrustStatus, projectTrustReloadOptions, trustProject } from "./project-trust.ts";
...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  assert.equal(
    Array.from(rpcSource.matchAll(/this\.syncProjectTrust\(\);\s*await this\.inner\.reload/g))
      .length,
    2,
  );
