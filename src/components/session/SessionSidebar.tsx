...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention }: Props) {
  ...
  }, [selectedIds, onSessionDeleted, loadSessions]);
  ...
  const handleBatchArchive = useCallback(async () => {
    ...
    setBatchActionPending(true);
    try {
      const res = await fetch("/api/sessions/archive-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids }),
      });
  ...
  const handleBatchUnarchive = useCallback(async (ids: string[]) => {
    setBatchActionPending(true);
    try {
      const res = await fetch("/api/sessions/unarchive-batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids }),
      });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {!showArchived && (
              <button
                ...
                title={batchMode ? "Exit batch mode" : "Batch operations"}
                style={{
                  display: "flex", alignItems: "center", justifyContent: "center",
                  background: batchMode ? "rgba(37,99,235,0.12)" : "var(--bg-hover)",
                  border: `1px solid ${batchMode ? "rgba(37,99,235,0.4)" : "var(--border)"}`,
            ...
            <button
              onClick={showArchived ? handleHideArchived : handleShowArchived}
              title={showArchived ? "Back to sessions" : "View archived"}
              style={{
                display: "flex", alignItems: "center", justifyContent: "center",
                background: showArchived ? "rgba(37,99,235,0.12)" : "var(--bg-hover)",
                border: `1px solid ${showArchived ? "rgba(37,99,235,0.4)" : "var(--border)"}`,
      ...
      {/* Session list */}
      <div style={{ flex: explorerOpen && (selectedCwdProp || selectedCwd) ? "1 1 0" : "1 1 auto", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 80 }}>
            ...
            <>
              ...
              {archivedSessions.map((session) => (
                <ArchivedItem
                  key={session.id}
                  session={session}
                  onRestore={() => handleBatchUnarchive([session.id])}
        ...
        {batchMode && !showArchived && selectedIds.size > 0 && (
          <div style={{
            padding: "8px 10px",
            ...
            <button
              ...
              style={{
                ...
                color: "var(--text-muted)",
                fontSize: 12,
                cursor: batchActionPending ? "not-allowed" : "pointer",
                opacity: batchActionPending ? 0.6 : 1,
              }}
            >
    ...
    <div>
      <div style={{ position: "relative" }}>
        ...
        <SessionItem
          ...
          batchMode={batchMode}
          isSelectedId={selectedIdsSet?.has(node.session.id) ?? false}
...
function SessionItem({
  ...
  return (
    <div
      onClick={confirmDelete || renaming ? undefined : batchMode ? () => onToggleSelect?.(session.id) : onClick}
    ...
    >
      {confirmDelete ? (
        ...
        <>
          ...
          {batchMode && (
            <div
              ...
              style={{
                width: 16, height: 16,
                borderRadius: 4,
                border: `1.5px solid ${isSelectedId ? "var(--accent)" : "var(--text-dim)"}`,
                background: isSelectedId ? "var(--accent)" : "transparent",
                display: "flex", alignItems: "center", justifyContent: "center",
                flexShrink: 0,
                cursor: "pointer",
              }}
          ...
          {/* Action buttons — shown on hover, hidden in batch mode */}
          {hovered && !batchMode && (
