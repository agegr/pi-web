...
import { normalizeToolCalls } from "@/lib/session/normalize";
import { sendAgentCommand } from "@/lib/agent/agent-client";
import type { ToolEntry } from "@/components/settings/ToolPanel";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadTools = useCallback(async (sid: string) => {
    try {
      ...
      if (tools) {
        const { getPresetFromTools } = await import("@/components/settings/ToolPanel");
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[]) => {
    ...
    try {
      if (isNew && newSessionCwd) {
        ...
        const { PRESET_NONE, PRESET_DEFAULT, PRESET_FULL } = await import("@/components/settings/ToolPanel");
