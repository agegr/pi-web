...
import { archiveSession } from "@/lib/session/session-utils";
...
export async function POST(req: Request) {
  try {
    ...
    const results = await Promise.allSettled(ids.map((id) => archiveSession(id)));
    ...
    return NextResponse.json({ ok: true, archived: ids.length - failed.length, failed });
  } catch (error) {
