...
| `PI_WEB_PASSWORD` | Зафиксировать пароль Basic Auth (имя пользователя всегда `pi`). Если не задан, сервер автоматически генерирует 6-значный PIN при первом сетевом запросе и сохраняет его в `~/.pi-web/` (переживает перезапуски). | Авто-генерируемый 6-значный PIN |
...
Привязка к адресу, отличному от loopback, открывает сетевой доступ к агенту, способному выполнять операции с широкими полномочиями. Basic Auth не шифрует пароль при передаче, поэтому не публикуйте Pi Web в интернете по обычному HTTP; используйте HTTPS через доверенный обратный прокси или надёжное VPN-подключение. Если обратный прокси передаёт внешнее имя хоста, добавьте это точное имя в `PI_WEB_ALLOWED_HOSTS`. Этот список разрешённых имён не меняет адрес, к которому привязывается Pi Web.

По умолчанию **пароль не задан**. При первом сетевом запросе к Pi Web сервер автоматически генерирует 6-значный PIN и сохраняет его в рантайм-хранилище (переживает перезапуски). Откройте иконку **Connect Phone** в верхней панели, чтобы увидеть текущий PIN; QR-код в этом окне также содержит одноразовый токен сопряжения, поэтому телефон попадает в Pi Web уже авторизованным, без ввода пароля.

Чтобы задать собственный пароль вместо автоматически сгенерированного, установите переменную окружения `PI_WEB_PASSWORD`:
...
**Та же сеть Wi-Fi.** Привяжитесь ко всем интерфейсам и откройте на телефоне `http://<локальный-IP-компьютера>:30141`:

```bash
pi-web --hostname 0.0.0.0
```

QR-код в модальном окне **Connect Phone** в верхней панели кодирует адрес и одноразовый токен сопряжения, поэтому телефон попадает прямо в сессию компьютера, **без ввода пароля**.
...
При использовании `-H 0.0.0.0` лаунчер автоматически получает Tailscale-IP и встраивает его в QR-код. Клиенты Tailscale: [iOS](https://apps.apple.com/us/app/tailscale/id1470499037) · [Android](https://play.google.com/store/apps/details?id=com.tailscale.ipn). Чтобы поделиться с кем-то ещё, пригласите его в тот же tailnet и передайте адрес.

**Аутентификация на телефоне.** 6-значный PIN запрашивается только если телефон заходит по URL напрямую (без сканирования QR-кода). QR-путь обходит этот шаг — токен сопряжения при первом сканировании обменивается на 30-дневный сессионный cookie. Cookie переживает перезапуски сервера (ключ подписи хранится в `~/.pi-web/session.key`), но становится недействительным при изменении значения `PI_WEB_PASSWORD` сервера.
