...
如果你把 pi-web 暴露到公网，建议设置 `PI_WEB_PASS` 来启用密码页：

```bash
PI_WEB_PASS='你的高强度密码' pi-web
```

如果只是本机或内网使用，不设置 `PI_WEB_PASS` 会保持当前的无密码行为。
...
- **公网部署**：如果 pi-web 可以被公网访问，建议设置 `PI_WEB_PASS`，避免未授权用户直接进入 Web UI。
