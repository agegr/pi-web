...
export async function DELETE(
  ...
  try {
    ...
    // An empty runtime session has no recoverable file. Preserve normal delete
    // semantics for it, including when shutdown writes its first empty file.
    const wasPersisted = existsSync(filePath);
    ...
    // Scan after stopping the selected session so it cannot persist a new
    // subagent between family discovery and moving the files.
